...
Ikaros is still pre-MVP. The agent loop, session store, context engine, memory
...
coding workflow, replay a failed turn, and understand the provider/agent state
without reading raw JSON or separate debug files.

Recent code work is also moving large CLI and agent files into smaller modules.
The current architecture migration has collapsed legacy implementation crates
into the target ownership model: `ikaros-state`, `ikaros-execution`,
`ikaros-providers`, `ikaros-agent`, `ikaros-surfaces`, and
`ikaros-terminal`. New top-level dependencies should use these architecture
crates directly.

## What Exists Today

- `ikaros` and `ikaros chat` without `--message` open the same default
  terminal chat surface. There is no separate top-level `tui` or `workbench`
  mode; `ikaros <PATH>` selects the workspace.
...
- `ikaros-host` centralizes config-driven host assembly instead of requiring
  each CLI, chat, gateway, or task entry point to rebuild sessions and
  registries independently.
- `ikaros-execution` centralizes reusable skill/tool contracts, execution
  interfaces, policy, approvals, audit, toolset metadata, scheduling
  descriptors, workspace filesystem scope, process runners, dry-run/Docker
  backends, governed network egress, and sandbox diagnostics.
...
- Keep `ikaros-state::session` as the runtime fact source for chat, gateway,
...
- Keep `ikaros-protocol::context` as the shared boundary for context bundles, references,
...
- Keep the terminal workbench as the primary pre-MVP user surface. The baseline
  now includes `ikaros`, raw-mode input, multiline editing,
  mouse-wheel scrolling, command palette, navigable live cells, approval
  overlays, quiet routing for inspect/action/probe commands, context chips in
  the composer model, cancellation, session resume, and `/code ...` workflow
  controls. Remaining work is hardening: PTY-backed smoke tests, better resize
  behavior, clearer recovery prompts, auditing any remaining protocol lines in
  human mode, and stable structured models for ACP or future GUI consumers.
