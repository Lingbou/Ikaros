...
use super::super::{interactive::InteractiveChatRuntime, workbench};
use super::activity::verbose_live_cells_enabled;
use anyhow::Result;
use ikaros_providers::model::ModelStreamEvent;
use ikaros_state::session::{
    AgentEvent, AgentEventKind, SessionId, SessionStore, SqliteSessionStore, TurnId,
};
use ikaros_terminal::terminal_inline;
pub(in crate::chat) fn print_live_event_cells(
...
pub(super) fn live_cell_snapshot(events: &[AgentEvent]) -> String {
...
#[cfg(test)]
pub(in crate::chat) fn compact_live_event_cells(
    events: &[&AgentEvent],
) -> Vec<workbench::WorkbenchCell> {
    compact_live_event_cells_with_debug(events, false)
}

pub(in crate::chat) fn compact_live_event_cells_with_debug(
    events: &[&AgentEvent],
    ...
    for event in events {
        ...
        match stream_event {
            ModelStreamEvent::Start {
                ...
                provider = terminal_inline(event_provider);
                model = terminal_inline(event_model);
            }
...
pub(in crate::chat) fn live_cells_json_line(
    events: &[&AgentEvent],
...
pub(in crate::chat) fn default_live_cell_event(kind: &AgentEventKind) -> bool {
    matches!(
        kind,
        AgentEventKind::ToolCallCompleted
            | AgentEventKind::ToolCallFailed
