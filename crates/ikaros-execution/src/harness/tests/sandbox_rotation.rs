...
fn sandbox_debug_report_exposes_available_and_planned_isolation_levels() {
    let matrix = crate::harness::sandbox_isolation_matrix();
    assert!(matrix.iter().any(|entry| {
        entry.level == crate::harness::SandboxIsolationLevel::WorkspaceOnly
            && entry.status == crate::harness::SandboxIsolationStatus::Available
    }));
    assert!(matrix.iter().any(|entry| {
        entry.level == crate::harness::SandboxIsolationLevel::NetworkRestricted
            && entry.status == crate::harness::SandboxIsolationStatus::Available
    }));
    assert!(matrix.iter().any(|entry| {
        entry.level == crate::harness::SandboxIsolationLevel::Container
            && entry.status == crate::harness::SandboxIsolationStatus::Available
    }));

    let local = crate::harness::local_sandbox_debug_report("local", true, None);
    assert_eq!(
        local.level,
        crate::harness::SandboxIsolationLevel::NetworkRestricted
    );
    ...
    let dry_run = crate::harness::local_sandbox_debug_report("dry-run", false, None);
    assert_eq!(dry_run.level, crate::harness::SandboxIsolationLevel::DryRun);
    assert_eq!(dry_run.network_egress, "deny_by_default");
    assert_eq!(dry_run.host_allowlist_mode, "deny_by_default");

    let docker =
        crate::harness::local_sandbox_debug_report("docker", true, Some("rust:1.85-bookworm"));
    assert_eq!(
        docker.level,
        crate::harness::SandboxIsolationLevel::Container
    );
    assert_eq!(
