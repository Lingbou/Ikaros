...
async fn run_chat_message_creates_relationship_candidate_from_clear_user_preferences() {
    ...
    assert!(
        snapshot.notes[0]
            .tags
            .iter()
            .any(|tag| tag == "pending-candidate"),
        "pending relationship candidates should be visible to chat context without promoting core memory"
    );

    let remembered = run_chat_message(
        "What do I prefer?",
        &paths,
        &workspace,
        Some("build"),
    ...
    assert_eq!(remembered.relationship_hits, 1);
...
async fn run_chat_message_uses_explicit_mock_provider_for_offline_runtime_paths() {
    ...
    let (session, registry) =
        ikaros_host::session_and_registry(&paths, &workspace, Some("build")).expect("session");
    session
        .execute_skill(
            &registry,
            "memory_append",
