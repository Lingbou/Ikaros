...
- `crates/ikaros-core`: shared config schemas, paths, task state, redaction,
  errors, and agent profile types.
...
- `crates/ikaros-context`: context bundles, prompt sections, references,
  ...
  diff primitives.
- `crates/ikaros-gateway`: local message inbox/outbox metadata, delivery
  routes, gateway adapter/webhook/admin logic, and gateway-local policy checks.
- `crates/ikaros-harness`: policy engine, approval queue, audit log,
  execution sessions, plugins, guardrails, and task runner.
- `crates/ikaros-host`: host-side assembly for runtime locations, agent
  instances, execution sessions, skill registries, sandbox backends, provider
  and store wiring, and governed egress.
...
- `crates/ikaros-protocol`: stable versioned wire/session protocol types shared
  by API, TUI, gateway, replay, and external surfaces.
...
- `crates/ikaros-runtime`: runtime execution for chat, tasks, schedules,
  gateway drain, body status, diagnostics, and agent handoff over dependencies
  assembled by `ikaros-host`.
- `crates/ikaros-sandbox`: concrete execution environments, workspace
  filesystem scope, process runners, dry-run/Docker backends, governed network
  egress, and sandbox diagnostics.
...
- `crates/ikaros-skills`: one crate for built-in skills, organized internally
  by groups/packs for filesystem, shell/git, memory, RAG, voice, coding,
  persona, plugins, and progressive-disclosure tool bridges.
- `crates/ikaros-soul`: persona, emotion, tone, and relationship primitives.
- `crates/ikaros-toolkit`: shared skill/tool contracts, execution interfaces,
  tool registries, audit event shapes, toolset metadata, and scheduling
...
Planned refactor seams not present as crates in this worktree: `ikaros-api`
should own the local API server adapter when it lands, and `ikaros-tui` should
own pure TUI render/input behavior. Until those crates exist, docs should name
them as planned ownership rather than list `crates/ikaros-api` or
`crates/ikaros-tui` as current directories.
