...
| 记录 | 创建时间 | 保存内容 |
| --- | --- | --- |
| `RunConfig` | 用户提交、Run 入队时 | 会话及用户 Item 标识、工作区、模型与公开配置指纹、工具定义、身份与技能目录、模型窗口和运行预算 |
...
| `maxModelCalls` | 100 次 | 本 Run 的模型调用总上限 |
| `maxDurationSeconds` | 3,600 秒 | 本 Run 的执行时间上限 |

模型窗口和输出上限可以在模型设置中修改；Run 提交后使用冻结值。
输出预留必须大于零且小于模型窗口，调用次数和运行时间必须为正整数。
Provider Adapter 每次调用只发送一次请求，不在内部隐式重试。
...
Session 使用当前 schema 10；Journal 只接受 schema 7。
...
- 模型窗口、输出上限和 Run 预算冻结；非法配置在执行前拒绝。
