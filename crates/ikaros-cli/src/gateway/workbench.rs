...
pub(crate) fn run_gateway_adapter_workbench_command(
    ...
    match args {
        ...
        ["enqueue", rest @ ..] => {
            ...
            enqueue_gateway_adapter_message(enqueue, &paths.gateway_dir)?;
        }
        ["render-delivery" | "render_delivery", rest @ ..] => {
            let render = parse_message_adapter_render_delivery_args(rest)?;
            render_gateway_adapter_delivery(render, &paths.gateway_dir)?;
        }
...
pub(in crate::gateway) fn parse_message_daemon_start_args(
    args: &[&str],
...
pub(in crate::gateway) fn parse_message_daemon_stop_args(
    args: &[&str],
...
pub(in crate::gateway) fn print_gateway_daemon_workbench_usage() {
    println!(
        "gateway_daemon_usage: /gateway daemon [status|start|stop|restart] [--interval-seconds N] [--limit N] [--reason TEXT]"
    );
}

pub(in crate::gateway) fn parse_message_adapter_enqueue_args(
    args: &[&str],
...
pub(in crate::gateway) fn parse_message_adapter_render_delivery_args(
    args: &[&str],
...
pub(in crate::gateway) fn next_workbench_arg<'a>(
    args: &'a [&str],
    index: usize,
    option: &str,
) -> Result<&'a str> {
...
pub(in crate::gateway) fn parse_message_kind_arg(value: &str) -> Result<MessageKindArg> {
...
pub(in crate::gateway) fn print_gateway_adapter_workbench_usage() {
