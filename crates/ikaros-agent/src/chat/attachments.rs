...
use ikaros_providers::model::ModelContentBlock;
use std::{
    fs,
    path::{Path, PathBuf},
};
...
pub fn content_block_from_parts(kind: &str, value: &str) -> Result<ModelContentBlock> {
...
pub fn content_block_kind(block: &ModelContentBlock) -> &'static str {
    match block {
...
pub fn content_block_summary(block: &ModelContentBlock) -> String {
    match block {
        ModelContentBlock::Text { text } => {
...
mod tests {
    ...
    #[test]
    fn content_blocks_from_args_infers_mime_types_and_file_names() {
        let blocks = content_blocks_from_args(
            &["https://example.test/photo.PNG?download=1".into()],
            &["voice.mp3".into()],
            &["docs/report.md#section".into()],
        );
        ...
        assert!(matches!(
            ...
            ModelContentBlock::Image {
                ...
                && image_url == "https://example.test/photo.PNG?download=1"
        ));
        assert!(matches!(
            &blocks[1],
            ModelContentBlock::Audio {
                mime_type,
                audio_url,
            } if mime_type.as_deref() == Some("audio/mpeg") && audio_url == "voice.mp3"
        ));
        assert!(matches!(
            &blocks[2],
            ModelContentBlock::File {
                ...
                && name.as_deref() == Some("report.md")
        ));
    }

    #[test]
    fn local_workspace_attachment_is_inlined_as_data_url() {
        let temp = tempdir().expect("tempdir");
        let file = temp.path().join("note.txt");
        fs::write(&file, "hello attachment").expect("write attachment");

        let block = content_block_from_parts_resolving_path("file", "note.txt", temp.path())
            .expect("inline local workspace attachment");

        let ModelContentBlock::File {
            file_url,
            mime_type,
            ..
        } = block
        ...
        assert!(file_url.starts_with("data:text/plain;base64,"));
        assert!(file_url.ends_with("aGVsbG8gYXR0YWNobWVudA=="));
    }

    #[test]
    fn local_attachment_rejects_paths_outside_workspace() {
        let workspace = tempdir().expect("workspace");
        let outside = tempdir().expect("outside");
        let outside_file = outside.path().join("secret.txt");
        fs::write(&outside_file, "secret").expect("write outside file");

        let err = content_block_from_parts_resolving_path(
            "file",
            &outside_file.display().to_string(),
            workspace.path(),
        )
        .expect_err("outside attachment should fail");

        assert!(err.to_string().contains("outside workspace"));
    }

    #[test]
    fn content_block_summary_redacts_secrets_and_describes_data_urls() {
        let block = ModelContentBlock::Image {
            image_url: "data:image/png;base64,sk-secret-value".into(),
            mime_type: Some("image/png".into()),
            detail: Some("high".into()),
        };

        let summary = content_block_summary(&block);

        assert!(summary.contains("kind=image"));
        assert!(summary.contains("detail=high"));
        assert!(summary.contains("inline-data:image/png"));
