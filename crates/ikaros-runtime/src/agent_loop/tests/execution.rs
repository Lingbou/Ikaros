...
#[tokio::test]
async fn agent_loop_rejects_json_fallback_tool_not_in_direct_manifest() {
    let temp = tempfile::tempdir().expect("tempdir");
    let session = ExecutionSession::new(temp.path().join("workspace"), temp.path().join("audit"));
    let direct_calls = Arc::new(AtomicUsize::new(0));
    let deferred_probe = Arc::new(ConcurrencyProbe::default());
    let mut registry = SkillRegistry::new();
    registry.register_with_toolset(
        EchoSkill {
    ...
    let provider = SequenceProvider {
        ...
        responses: vec![
            r#"{"tool_calls":[{"name":"rag_probe","input":{"text":"guessed token=abc123"}}]}"#
                .into(),
            r#"{"final_answer":"handled rejected tool"}"#.into(),
        ],
    };

    let report = run_agent_loop(
        AgentLoopInput {
            session_id: Some("direct-manifest-session".into()),
            turn_id: Some("direct-manifest-turn".into()),
            task_id: Some("direct-manifest-task".into()),
            system_prompt: "Use tools when useful.".into(),
            user_input: "start".into(),
        },
        &provider,
        ...
        &registry,
        AgentLoopOptions {
            toolsets: ToolsetSelection::new([Toolset::Core, Toolset::Rag]),
            ..AgentLoopOptions::default()
        },
    )
    .await
    ...
    assert_eq!(report.tool_results[0].name, "rag_probe");
    assert!(!report.tool_results[0].ok);
    assert_eq!(
        report.tool_results[0].output["reason"],
        json!("tool_not_in_direct_manifest")
    );
    assert!(
        report.tool_results[0]
            .summary
            .contains("model-visible direct tool manifest")
    );
    assert_eq!(direct_calls.load(Ordering::SeqCst), 0);
    assert_eq!(deferred_probe.calls(), 0);
    let audit_events = session.audit.read_all().expect("audit");
    assert!(!audit_events.iter().any(|event| {
        event.kind == "tool_call"
            && event.data.get("name").and_then(serde_json::Value::as_str) == Some("rag_probe")
    }));
}
