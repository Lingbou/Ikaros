...
"#;
    ...
    #[test]
    fn config_loader_accepts_utf8_bom_prefix() {
        let raw = "\u{feff}schema_version: 1\n";

        let config = validation::load_yaml_shape_checked(raw).expect("shape with UTF-8 BOM");
        let report = IkarosConfig::validate_yaml(raw).expect("validate with UTF-8 BOM");
        ...
        assert!(
            !report
                .errors
                .iter()
                .any(|issue| issue.message.contains("more than one document")),
            "{report:#?}"
        );
    }
