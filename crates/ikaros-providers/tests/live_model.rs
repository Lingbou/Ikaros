...
use ikaros_providers::model::{
    GovernedModelProvider, ModelMessage, ModelProvider, ModelRequest, ModelRequestOptions,
    ModelRuntimeLimits, ModelUsageLedger, OpenAiCompatibleProvider,
};
use std::{
    env,
    path::{Path, PathBuf},
...
async fn live_generate_smoke(config: ModelConfig, provider_settings: RemoteProviderConfig) {
    ...
    let request = ModelRequest {
        messages: vec![ModelMessage::user("Reply with IKAROS_OK only.")],
