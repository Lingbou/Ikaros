    ...
    let test_patch = session
        .execute_skill(
            ...
            json!({
                ...
                "apply_patch": true,
                "diff": "diff --git a/note.txt b/note.txt\n--- a/note.txt\n+++ b/note.txt\n@@ -1 +1 @@\n-old\n+new\n",
                "test_commands": [{"command": "cargo test -p ikaros-execution", "reason": "focused"}],
            }),
        )
        .await
...
async fn code_workflow_edit_mode_applies_patch_through_execution_env() {
    ...
    let requested = session
        .execute_skill(
            ...
            json!({
                ...
                "test_commands": [{"command": "cargo test -p ikaros-execution", "reason": "coding execution changed"}],
            }),
        )
        .await
    ...
    assert_eq!(
        ...
        json!("cargo test -p ikaros-execution")
    );
    assert_eq!(
...
async fn code_workflow_persists_coding_turn_timeline_to_session_store() {
    ...
    let registry = builtin_registry(SkillEnvironment {
        coding_session: Some(CodingSessionConfig {
            ...
            model_provider: None,
            cancellation: ikaros_execution::harness::CancellationToken::new(),
        }),
...
async fn mock_model_coding_replay_fixture_persists_two_iteration_loop() {
    ...
    let context = ikaros_execution::coding_context::CodingTurnContext::from_workspace(
        ikaros_execution::coding_context::CodingTurnContextInput {
            workspace_root: workspace.clone(),
            objective: "mock model replay should preserve coding loop".into(),
            mode: ikaros_execution::coding_context::CodingMode::Edit,
            session_id: Some(session_id.as_str().to_owned()),
            turn_id: Some(turn_id.as_str().to_owned()),
            instructions: Vec::new(),
            permission_profile: ikaros_execution::coding_context::CodingPermissionProfile::default(
            ),
...
+pub fn value() -> i32 { 3 }
";
    let report = ikaros_execution::coding_runtime::MockModelCodingRuntime::default()
        .run_scripted_turns_with_env(
            ikaros_execution::coding_runtime::MockModelCodingInput {
                context,
                turns: vec![
                    ikaros_execution::coding_runtime::MockModelCodingTurn {
                        candidate_diff: Some(first_diff.into()),
                        test_matrix: vec![ikaros_execution::testing::TestFailureAnalyzer::analyze(
                            "cargo test",
                            101,
                            "test result: FAILED",
                            "",
    ...
    let result = session
        .execute_skill(
            ...
            json!({
                ...
                "mode": "test",
                "run_tests": true,
                "test_commands": [{"command": "cargo test -p ikaros-execution", "reason": "focused coding check"}],
            }),
        )
        .await
        .expect("coding workflow");
    ...
    let result = session
        .execute_skill(
            ...
            json!({
                ...
                "run_tests": true,
                "test_commands": [
                    {"command": "cargo test -p ikaros-execution", "reason": "focused coding check"},
...
async fn code_workflow_model_loop_cancels_while_waiting_for_provider() {
    ...
    let cancellation = ikaros_execution::harness::CancellationToken::new();
