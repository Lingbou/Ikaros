...
use super::interactive::{
    InteractiveChatStatusInput, available_agent_lines, cancel_selected_screen_continuation,
    clear_interactive_session, continuations_json_line, format_interactive_chat_status,
    should_emit_default_inline_stdout,
};
use super::{
...
use ikaros_providers::model::{ModelStreamEvent, ModelUsageLedger, TokenUsage};
...
fn default_interactive_chat_session_resumes_latest_matching_nonempty_session() {
    ...
    let selected = super::interactive_chat_session_id(&agent, None).expect("selected session");
    assert_eq!(selected, "newer-session");

    let explicit = super::interactive_chat_session_id(&agent, Some("manual-session"))
