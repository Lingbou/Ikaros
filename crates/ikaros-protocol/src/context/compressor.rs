...
use super::{
    ChatContext, ContextBudget, ContextCompressedSection, ContextDiff, ContextError,
    ContextLimitReport, ContextResult, PriorityContextEngine, TokenEstimator,
    chat_context_token_count, diff_chat_context,
};
...
impl TrajectoryCompressor {
    ...
    pub fn compress(
        ...
        if !budget.is_unbounded() && protected_tokens > budget.max_tokens {
            return Err(ContextError::limit_exceeded(ContextLimitReport {
...
#[cfg(test)]
mod tests {
    use super::super::{ContextSectionKind, HeuristicTokenEstimator};
    use super::*;
    ...
    fn compressor_preserves_protected_relationship_and_references() {
        ...
        assert!(report.compressed_sections.iter().all(|section| {
            !matches!(
                section.section,
                ContextSectionKind::Relationship | ContextSectionKind::References
            )
        }));
    }
    ...
    fn compressor_fails_when_protected_context_exceeds_budget() {
        ...
        match error {
            ContextError::LimitExceeded {
                ...
                assert!(protected_sections.contains(&ContextSectionKind::References));
            }
