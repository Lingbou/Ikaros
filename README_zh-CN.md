...
- `crates/ikaros-models`：mock、OpenAI-compatible、Anthropic 和 Ollama 模型 provider，以及 context profile、provider registry metadata、治理、retry policy、health state 和用量日志。
...
cargo run -p ikaros-cli -- chat # 进入 REPL 后输入：/code plan "review a candidate patch"
