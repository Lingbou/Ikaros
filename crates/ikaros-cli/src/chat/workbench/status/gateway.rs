...
use ikaros_protocol::{
    GatewayDelivery, GatewayDeliveryStatus, GatewayMessage, GatewayMessageStatus,
};
use ikaros_state::gateway::LocalGatewayStore;
use ikaros_state::session::gateway_session_id;
use std::{
    fs,
    path::{Path, PathBuf},
};
...
pub(in crate::chat) fn print_gateway_status_for_human(paths: &IkarosPaths) -> Result<()> {
    for line in gateway_status_human_lines(paths)? {
        println!("{line}");
    }
    Ok(())
}

pub(in crate::chat) fn gateway_status_human_lines(paths: &IkarosPaths) -> Result<Vec<String>> {
    let store = LocalGatewayStore::new(&paths.gateway_dir);
    let messages = store.list()?;
    let deliveries = store.deliveries()?;
    let pending = messages
    ...
    let (lock_present, lock_stale, lock_owner) = gateway_worker_lock_summary(&lock_path);

    let mut lines = vec![
        "* Gateway".to_owned(),
        format!("  inbox: {}", path_display(store.inbox_path())),
        format!("  outbox: {}", path_display(store.outbox_path())),
        format!("  messages: {pending} pending, {processing} processing, {processed} processed"),
        format!(
            "  failures: {failed} failed, {cancelled} cancelled, {dead_lettered} dead-lettered"
        ),
        format!(
            "  deliveries: {delivery_pending} pending, {delivery_processing} processing, {delivery_delivered} delivered, {delivery_dead_lettered} dead-lettered"
        ),
        format!("  worker lock: present={lock_present} stale={lock_stale}"),
    ];
    if lock_present == "true" {
        lines.push(format!("  owner: {lock_owner}"));
    }
    lines.push(
        "  controls: /gateway daemon status, /gateway daemon start, /gateway daemon stop"
            .to_owned(),
    );
    Ok(lines)
}
