...
use ikaros_agent::chat::{
    ChatHistoryRecord, ChatHistorySessionSummary, append_chat_history_delete_tombstone,
...
use ikaros_host::session_state_db_candidates;
...
pub(super) fn print_chat_history(
    ...
    let records = if let Some(session_id) = session_id {
        session_replay_chat_history_records(paths, workspace, agent_override, session_id)?
            .unwrap_or_default()
    } else {
        session_replay_all_chat_history_records(paths, workspace, agent_override)?
            .unwrap_or_default()
    };
...
fn session_replay_chat_history_records(
    ...
    for state_db in chat_state_db_candidates(paths, workspace, agent_override)? {
        if !state_db.is_file() {
            continue;
        }
...
fn chat_state_db_candidates(
    ...
    Ok(session_state_db_candidates(
        paths,
        workspace,
        agent_override,
        true,
...
pub(super) fn print_chat_sessions(
    ...
    let sessions = session_replay_chat_session_summaries(paths, workspace, agent_override, limit)?;
    println!("history_source: session_replay");
    println!("history_authority: session_store");
...
pub(super) fn clear_chat_history(
    ...
    let replay_deleted = mark_all_chat_history_sessions_deleted(paths, workspace, agent_override)?;
    println!("history_source: session_replay");
    println!("history_authority: session_store");
    println!("deleted_records: 0");
