...
- `ikaros-session`：`SessionId`、`TurnId`、typed `AgentEvent`、append-only session entry、`SessionStore`、SQLite `state.db` 和 replay 查询。
...
- `state.db`：session metadata、append-only session entry、已持久化的 agent-loop event、approval record 和 replay 数据。chat、gateway、schedule、memory 和 audit 更大范围迁入这个 store 的工作仍在进行。
