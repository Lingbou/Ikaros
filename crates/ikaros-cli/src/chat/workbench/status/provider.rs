...
pub(in crate::chat) fn model_status_human_lines(
    ...
    Ok(vec![
        "• Model".to_owned(),
        format!("  name: {}", terminal_inline(&model.model)),
        format!("  provider: {}", terminal_inline(&model.provider)),
        format!(
            "  profile: {} ({})",
        ...
        format!(
            "    reasoning: {}",
            model_capability(&descriptor, |descriptor| descriptor.capabilities.reasoning)
        ),
        format!(
            "    image: {}",
            model_capability(&descriptor, |descriptor| descriptor
                .capabilities
                .image_input)
        ),
        format!(
        ...
        format!(
            "    file: {}",
            model_capability(&descriptor, |descriptor| descriptor.capabilities.file_input)
        ),
        format!("  transport: {}", terminal_inline(&model.transport)),
...
pub(in crate::chat) fn provider_status_human_lines(
    ...
    lines.push(format!("  provider: {}", terminal_inline(&model.provider)));
    lines.push(format!("  model: {}", terminal_inline(&model.model)));
    lines.push(format!(
        "  profile: {} ({})",
        model_profile(&descriptor),
        model_profile_source(&model.provider, &model.compat_profile, descriptor.as_ref())
    ));
    lines.push(format!("  health: {}", terminal_inline(&provider_health)));
    lines.push(format!(
        "  streaming: {}",
        model_capability(&descriptor, |descriptor| descriptor.capabilities.streaming)
    ));
