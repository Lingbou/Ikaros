...
4. 模型 turn 通过 `AgentRuntime` 执行；默认实现是 `HarnessAgentRuntime`。
5. 工具 dispatch 必须经过 `ExecutionSession` 和 `ExecutionEnv`，不能绕过 harness 直接碰 host API。
6. Harness 评估策略，记录审计事件，然后执行、请求审批或拒绝。
7. Runtime 把结果转换成 CLI、body、schedule、gateway、chat 或 agent caller 可用的报告。
...
- `memory/`：本地记忆记录和 provider-backed memory 状态。
...
- `ModelProvider` 生成/stream 文本；`ModelTransport` 描述 provider wire format；`AgentRuntime` 拥有 turn loop。
