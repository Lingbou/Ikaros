...
The shared `ExecutionEnv`, filesystem, process, and network traits live in
`ikaros-execution::toolkit`. The concrete local, dry-run, Docker, workspace-scoped, and
network-governed implementations live in `ikaros-execution::sandbox`.
...
remote egress adapters implemented by RAG skills, not by `ikaros-state::rag`. They
