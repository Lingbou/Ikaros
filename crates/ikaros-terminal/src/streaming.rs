...
mod tests {
    ...
    fn stream_renderer_holds_table_until_finish() {
        ...
        assert!(stream.push_delta("| File | Status |\n").is_empty());
        ...
        assert!(finish.lines.iter().any(|line| line.contains("File")));
        assert!(
            finish
                .lines
                .iter()
    ...
    fn stream_renderer_finally_reflows_held_table() {
        ...
        assert!(stream.push_delta("| Name | Value |\n").is_empty());
        assert!(stream.push_delta("| --- | --- |\n").is_empty());
        assert!(stream.push_delta("| x | short |\n").is_empty());
        assert!(stream.push_delta("| long-name | very long |\n").is_empty());
        ...
        assert!(finish[3].contains("long-name | very long"));
    }
}
