...
`ikaros-protocol::context` owns the reusable primitives:
...
`ikaros-agent::chat::context_engine` owns orchestration around those
primitives. Chat calls governed safe-read skills for memory and RAG, resolves
workspace-local references, adds agent/tool guidance, renders the final system
prompt through `PromptBuilder`, sends cache-stable and dynamic system prompt
layers as separate model messages where supported, and emits the session event.
The default context engine is `deterministic`; `--context-engine llm-summary`
...
letting protocol context types depend on agent orchestration, execution, or
model providers.
