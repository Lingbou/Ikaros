...
fn windows_pid_is_running(pid: u32) -> bool {
    ...
    stdout
        .lines()
        .any(|line| tasklist_csv_pid(line).is_some_and(|task_pid| task_pid == pid))
}

#[cfg(target_os = "windows")]
fn tasklist_csv_pid(line: &str) -> Option<u32> {
...
mod tests {
    ...
    fn message_worker_lock_marks_nonexistent_pid_as_stale() {
        let contents = "pid=999999999\nstarted_at=2026-06-23T00:00:00Z\n";

        assert!(message_worker_lock_is_stale(contents));
    }

    #[test]
    fn message_worker_lock_keeps_current_pid_active() {
        let contents = format!(
            "pid={}\nstarted_at=2026-06-23T00:00:00Z\n",
            std::process::id()
        );
