...
pub(crate) fn json_string(value: &serde_json::Value, key: &str, default: &str) -> String {
...
pub(crate) fn screen_overlay_routing_json(
    active_view: &str,
...
pub(crate) fn draw_command_palette_overlay(
    frame: &mut Frame<'_>,
    ...
    frame.render_widget(
        Paragraph::new(command_palette_overlay_text(screen, state))
            .block(
                Block::default()
...
pub(crate) fn command_palette_summary_line(
    screen: &WorkbenchScreen,
    state: &WorkbenchScreenState,
) -> String {
    let popup = command_palette_overlay_json(screen, state);
    format!(
        "palette query={} selected={}/{} command={} action={} up/down=move enter=open esc=close inspect=/commands",
        ...
        json_string(&popup, "selected_action_label", "Run command"),
    )
}

pub(crate) fn command_palette_overlay_text(
    screen: &WorkbenchScreen,
    state: &WorkbenchScreenState,
) -> String {
    let popup = command_palette_overlay_json(screen, state);
    ...
    for item in items.into_iter().take(10) {
        ...
        lines.push(format!(
            ...
            json_string(&item, "action_label", "Run command"),
        ));
    }
    lines.push(String::new());
...
pub(crate) fn draw_approval_modal_overlay(frame: &mut Frame<'_>, screen: &WorkbenchScreen) {
...
pub(crate) fn approval_risk_breakdown_text(value: &serde_json::Value) -> String {
    format!(
