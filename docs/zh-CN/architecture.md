...
Ikaros 是 persona-first 的本地 Agent Runtime。核心边界是：runtime 编排 turn，
harness 治理工具调用，provider adapter 只处理模型 wire format，本地状态默认留在
`IKAROS_HOME`。
...
- Host assembly：加载 config、解析 `AgentInstance`、构造 `RuntimeLocation`，
  并为 runtime caller 组装 `ExecutionSession`、sandbox-backed `ExecutionEnv`
  和 `SkillRegistry` 的边界。
- Harness：每次受治理工具调用的策略、审批、审计和执行 session 边界。
- Tool contracts：`ikaros-toolkit` 提供的共享 skill registry、tool descriptor、
  process、filesystem、network 和 audit shape。
- Sandbox：`ikaros-sandbox` 提供的具体 local、dry-run、Docker、workspace 和
  受治理网络执行 backend。
...
- `ikaros-context`：context bundle、prompt builder/section、reference、provider-aware token budget、
  ...
  request builder。
- `ikaros-runtime`：在 host 已经组装好的依赖之上执行工作，包括诊断、聊天、任务、计划任务、
  gateway drain、body frame、agent handoff、`AgentRuntime`、`AgentHarness` 和
  context 编排。Runtime 应接收 `RuntimeLocation`、session/store handle、provider、
  registry 和 execution environment；不应加载 config、发现 agent 或组装 host 资源。
- `ikaros-host`：为 runtime caller 组装 environment、registry、provider 和 store。
  它负责 `RuntimeLocation`、`AgentInstance` 解析、`ExecutionSession`、
  `SkillRegistry`、skill environment、sandbox-backed runtime `ExecutionEnv`、
  provider/search 网络出口 allowlist，以及其他 config-driven host wiring。
- `ikaros-harness`：策略判断、审批请求、审计日志、`ExecutionSession`、插件、
  guardrail 和任务 runner。它组合共享 tool contract 和 sandbox backend，不再直接
  拥有这些实现。
- `ikaros-toolkit`：共享 skill/tool 契约、registry、执行接口、audit event shape、
  toolset metadata 和工具调度 descriptor，供 harness、skills、runtime 和 sandbox
  adapter 复用。
- `ikaros-sandbox`：具体执行环境、workspace 文件系统边界、进程 runner、
  dry-run/Docker backend、受治理网络出口、network policy 和 sandbox 诊断。
...
- `ikaros-protocol`：CLI、TUI、gateway、本地 API、replay 和外部集成界面共享的稳定版本化
  wire/session protocol 类型。它只放 durable product-facing shape；随着类型族增长，按
  surface 或 session family 拆成 crate 内部模块。
- `ikaros-gateway`：本地 inbox/outbox store、lease/retry/dead-letter metadata、
  gateway protocol policy、adapter mapping、webhook ingress helper 和 admin operation。
  它不能依赖 runtime 或 CLI。稳定的跨界 gateway/session wire shape 属于
  `ikaros-protocol`；gateway-local store record 和 queue policy 留在这里。
- `ikaros-voice`：mock 和 OpenAI-compatible TTS/ASR provider。
- `ikaros-skills`：内置 skill implementation 的单一 crate，通过共享 tool contract
  暴露，并由 harness 治理。Skill 在 crate 内部按 group 或 pack 组织；不要在稳定复用
  seam 出现前拆成许多 crate。
- `ikaros-cli`：`ikaros` binary 的轻量 `clap`、dispatch 和 terminal adapter。
  它负责解析 flag、调用 host/runtime/gateway/API/TUI adapter，并渲染终端输出；不拥有
  runtime execution、host assembly、gateway queue logic 或 protocol schema。
- `ikaros-body`：body/status/frame 合约和简单渲染器。
- `ikaros-automation`：本地计划任务元数据和运行状态。
- `ikaros-service`：本地 worker 进程的 service manager 模板渲染。
- `ikaros-coding`：repo scan、guarded patch、结构化 patch failure、
  turn diff tracking、code review、coding turn report、self-modify record 和
  test-command analysis。
- `ikaros-soul`：persona、情绪、语气和关系模型基础类型。

当前 worktree 中尚未出现的计划 crate seam：

- `ikaros-api`：引入时应拥有本地 API server adapter：HTTP route binding、
  OpenAI-compatible request/response translation、bearer-token/rate-limit/audit
  adapter、protocol metadata endpoint 和 server lifecycle。它应复用 `ikaros-host`、
  `ikaros-runtime`、`ikaros-session` 和 `ikaros-protocol`；不应成为第二套 host
  assembly，也不应拥有 CLI/TUI/gateway queue 行为。
- `ikaros-tui`：引入时应拥有纯 TUI screen model、rendering、input handling、
  key binding 和 terminal interaction state。它应消费 protocol/session/runtime report
  并调用窄 adapter；不应拥有 runtime execution、host assembly、gateway drain、
  provider setup 或持久化 store。

## 重构迁移清单

跨 crate seam 移动代码时使用这份清单：

- [ ] 保持 `ikaros-cli` 是轻量 command 和 terminal adapter；可复用 command 行为应移动到
      host/runtime/gateway/API/TUI crate，而不是继续扩大 CLI module。
- [ ] `ikaros-api` crate 落地后，API server code 应进入这个 seam；在那之前，只把它记录为计划所有权，
      不要把缺失的 workspace crate 写成已经存在。
- [ ] `ikaros-tui` crate 落地后，TUI render/input code 应进入这个 seam；terminal UI
      代码不应组装 runtime dependency，也不应 drain gateway queue。
- [ ] 稳定的 wire/session/event shape 保持在 `ikaros-protocol`；gateway frame shape
      在作为 API、TUI、replay 或 adapter 共享契约暴露前，应迁移到那里。
- [ ] 保持 `ikaros-gateway` 不依赖 runtime 和 CLI。Gateway 拥有 queue store、lease、
      adapter mapping、webhook ingress helper、admin operation 和 protocol policy check；
      runtime 拥有消息执行。
- [ ] 保持 `ikaros-runtime` 只在已组装依赖之上执行 chat/task/schedule/gateway work。
      如果 runtime 代码加载 config、解析 agent、创建 registry 或选择 store，应迁移到
      `ikaros-host`。
- [ ] 保持 `ikaros-host` 只负责 environment、registry、provider、sandbox 和 store
      assembly。Host 不拥有 agent-loop 行为或 terminal rendering。
- [ ] 保持 `ikaros-skills` 是一个带内部 group/pack 的 crate，直到真实复用 seam 证明需要新 crate。
- [ ] 随着 protocol family 增长，`ikaros-protocol` 在 crate 内部模块化；不要把 provider、
      store、runtime 或 CLI implementation detail 放进去。
...
5. 工具 dispatch 必须经过 `ExecutionSession` 和挂载的 `ExecutionEnv`。Runtime 代码
   不能直接碰 host API，不能绕过 `ikaros-toolkit`，也不能重复实现 host assembly。
...
- 工具治理属于 harness，可复用工具契约属于 `ikaros-toolkit`，具体的
  filesystem/process/network adapter 属于 `ikaros-sandbox`。模型 provider 和 UI
  不能直接执行工具。`ikaros-host` 会用当前配置的 sandbox backend 组合出 runtime
  environment。进程执行会清空宿主环境、恢复一小组基线 allowlist、叠加显式 request
  env，并对敏感 env 诊断脱敏。
