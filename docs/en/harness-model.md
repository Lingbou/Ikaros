...
state. The reusable tool contracts live in `ikaros-execution::toolkit`; concrete execution
environments live in `ikaros-execution::sandbox`. Code outside the harness should not open
...
- `ExecutionEnv`: shared environment interface from `ikaros-execution::toolkit`, with local,
  dry-run, Docker, workspace, and network implementations in `ikaros-execution::sandbox`.
...
`ExecutionEnv` is the shared interface from `ikaros-execution::toolkit`. `ikaros-execution::sandbox`
