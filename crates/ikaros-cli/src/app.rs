...
fn init_tracing(paths: &IkarosPaths) -> Result<()> {
    ...
    let filter = tracing_subscriber::EnvFilter::try_from_default_env().unwrap_or_else(|_| {
        tracing_subscriber::EnvFilter::new(
            "warn,ikaros=info,ikaros_cli=info,ikaros_agent=info,ikaros_host=info,ikaros_providers=info,ikaros_execution=info,ikaros_skills=info,ikaros_state=info,ikaros_surfaces=info,ikaros_terminal=info",
        )
    });
