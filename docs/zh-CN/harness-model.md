...
`coding_progress` 和 `coding_result` 摘要，常见路径不需要先查 `debug coding-turn` JSON。它会组装 repo map、change plan、可选 patch attempt、
...
边界。文件、shell、coding、RAG maintenance、voice output、voice ASR 音频读取、self-modify 的 workspace 读写/check 和 command-backed plugin 都应走 session/env，不应直接调用 host API。
...
受控 wrapper 默认拒绝，只允许 allowlist 中精确解析出的 URL host，并会脱敏被拒绝的
host 摘要。Chat、task agent-loop 和 provider-backed coding 路径中的 provider HTTP
adapter 会拿到 egress-backed transport。任意 plugin 或 shell 代码不应绕过 harness
...
它保留 workspace 读取，但跳过写入和进程执行，并返回结构化 dry-run 输出。网络出口由
`execution.network.enabled` 和受控 allowlist 单独控制；如果 dry-run session 也必须避免网络副作用，需要关闭 `execution.network.enabled`。Docker 和 ssh 隔离仍是后续 backend 实现，不会被策略 metadata 暗示为已经存在。
