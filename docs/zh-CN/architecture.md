...
- Context primitive 属于 `ikaros-context`。Runtime chat 会把 relationship、显式 reference、history、memory 和 RAG 组装成 provider-aware token-budgeted `ContextBundle`。Provider metadata 会收窄可用 context window；token 计数在 native tokenizer adapter 接入前仍是启发式。
