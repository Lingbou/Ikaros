...
use ikaros_execution::harness::{
    CancellationToken, ExecutionSession, SkillRegistry, ToolExecutionMode,
};
use ikaros_state::session::AgentEventId;
use serde_json::json;
...
pub(super) async fn dispatch_scheduled_tool_batch(
    ...
    for (scheduled, started_event_id) in started {
        ...
        let direct_manifest = context.direct_manifest.to_vec();
