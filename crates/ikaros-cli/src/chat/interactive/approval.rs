    ...
    match args.as_slice() {
        [] | ["list"] => {
            if runtime.default_inline_stdout() {
                print_approval_status_for_human(runtime)?;
            } else {
        ...
        ["approve", id, rest @ ..] => {
            ...
            if runtime.default_inline_stdout() {
                print_approval_resume_for_human(runtime)?;
            } else {
        ...
        _ => {
            if runtime.default_inline_stdout() {
                println!("• Approval");
                println!("  usage: /approval approve <id> [note] | deny <id> [note]");
                print_approval_status_for_human(runtime)?;
            } else {
...
fn print_approval_status_for_human(runtime: &InteractiveChatRuntime) -> Result<()> {
...
fn print_approval_resume_for_human(runtime: &InteractiveChatRuntime) -> Result<()> {
    let pending = runtime.session.pending_approvals()?;
    if let Some(next) = pending.first() {
        println!(
            "  next approval: /approval approve {}",
