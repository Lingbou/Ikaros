...
fn budget_config(paths: &IkarosPaths, args: ConfigBudgetArgs) -> Result<()> {
    ...
    let changed = args.set.is_some() || args.disable;
    let after = if args.disable {
        None
    } else {
        args.set.or(before)
    };
    ...
    let current = if changed { after } else { before };
    if args.json {
        let payload = json!({
            "path": paths.config.display().to_string(),
            "changed": changed,
            "daily_token_budget_before": before,
            "daily_token_budget": current,
        ...
        println!(
            ...
            display_optional_budget(before)
        );
        println!("daily_token_budget: {}", display_optional_budget(current));
