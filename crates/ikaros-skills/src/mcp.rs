...
use ikaros_execution::toolkit::{PolicyRequest, ProcessRequest, Skill, SkillContext, SkillOutput};
...
impl Skill for McpStdioProbeSkill {
    ...
    async fn execute(&self, input: Value, ctx: SkillContext) -> Result<SkillOutput> {
        ...
        let stdin = ikaros_protocol::mcp::mcp_stdio_probe_input()?;
        let output = ctx
            .session
            .env
            .run_process(
        ...
        let mut report = ikaros_protocol::mcp::parse_mcp_stdio_probe_output(&output.stdout);
...
fn filter_probe_tools(
    tools: &mut Vec<ikaros_protocol::mcp::McpTool>,
