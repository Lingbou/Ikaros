...
The message gateway is a local inbox/outbox and adapter boundary for external
messages. Ingestion does not execute work; worker/drain paths later process
messages through agent and execution flows. Stable cross-surface wire shapes
belong in `ikaros-protocol`; queue state lives in `ikaros-state::gateway`, and
adapter mapping, webhook ingress, admin operations, and gateway-local policy
checks live in `ikaros-surfaces::gateway`.
...
Gateway-local queue records live in `ikaros-state::gateway`. Adapter policy and
webhook mapping live in `ikaros-surfaces::gateway`. Stable cross-surface wire
shapes that need to be shared by API, TUI, replay, and future adapters belong
in `ikaros-protocol`.
...
`message drain` and `message worker` process pending inbox records through
`ikaros-agent::gateway_drain` with dependencies assembled by the CLI/host
boundary.
...
permissions; agent/profile data only affects agent context and policy overlay.
...
a gateway-scoped user entry, an agent result entry, and typed start/end/error
...
Webhook messages are marked `safe_tools` by default. During gateway drain, chat
...
- Stable gateway/session wire shapes live in `ikaros-protocol`; adapters should
  depend on those shapes and treat `ikaros-state::gateway` records as local
  queue state. Adapters should still target the inbox/outbox queue, not agent
  internals.
- Session source identifies the external conversation; `agent` selects agent
