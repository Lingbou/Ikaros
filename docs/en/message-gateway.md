...
messages through runtime and harness flows. Stable cross-surface wire shapes
belong in `ikaros-protocol`; `ikaros-gateway` owns queue state, adapter mapping,
webhook ingress, admin operations, and gateway-local policy checks.
...
Gateway-local queue records and adapter policy live in `ikaros-gateway`. Stable
cross-surface wire shapes that need to be shared by API, TUI, replay, and future
adapters belong in `ikaros-protocol`. During the refactor, any similarly named
frame types still present in `ikaros-gateway` are implementation details until
they are moved or projected into the protocol crate.
...
- Stable gateway/session wire shapes live in `ikaros-protocol`; adapters should
  depend on those shapes once exposed and treat `ikaros-gateway` frame/store
  types as gateway-local until migrated. Adapters should still target the
  inbox/outbox queue, not runtime internals.
