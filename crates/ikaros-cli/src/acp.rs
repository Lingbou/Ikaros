...
use ikaros_agent::chat::new_chat_session_id;
use ikaros_core::{IkarosPaths, redact_secrets};
use ikaros_host::{host_agent_context, runtime_harness};
use ikaros_state::session::{
    AgentEvent, AgentEventKind, SessionEntry, SessionEntryKind, SessionId, SessionSource,
    SessionStore, SqliteSessionStore,
};
use ikaros_surfaces::mcp::{JsonRpcError, JsonRpcRequest, JsonRpcResponse};
...
async fn acp_session_prompt(request: &JsonRpcRequest, state: &AcpServerState) -> Result<Value> {
    ...
    let options = ChatRunOptions {
        ...
        session_source: Some(SessionSource::Service { name: "acp".into() }),
        stream: request
            .params
            .get("stream")
    ...
    let result = crate::chat::run_single_chat_message(
        &message,
        &state.paths,
        Path::new(&workspace),
        Some(&agent),
    ...
    match replay {
        Some(replay) => {
            let approvals: Vec<Value> = replay
                .approvals
                .iter()
                .filter(|a| matches!(a.status, ikaros_state::session::ApprovalStatus::Requested))
                .map(|a| {
                    json!({
                        "id": a.approval_id,
