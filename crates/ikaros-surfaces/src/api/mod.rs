...
use ikaros_core::{
    AgentInstance, IkarosConfig, IkarosPaths, ModelConfig, StructuredTraceEvent,
    StructuredTraceLog, redact_secrets,
};
use ikaros_execution::harness::{AuditEvent, AuditLog};
use ikaros_host::{
    api_embedding_services_shape_checked, api_model_services_shape_checked,
    host_agent_context_shape_checked, send_model_json_request_for_instance,
    send_provider_bytes_request_for_instance, send_provider_json_request_for_instance,
};
use ikaros_providers::model::{
    ModelContentBlock, ModelMessage, ModelRequest, ModelRequestOptions, ModelStream, ModelToolCall,
    ModelToolDefinition, TokenUsage, model_request_options_from_config,
};
...
impl Default for ApiServeOptions {
    fn default() -> Self {
        Self {
...
pub fn serve_api(
    ...
    server::serve_api_inner(options, paths, workspace, agent_override)
}
