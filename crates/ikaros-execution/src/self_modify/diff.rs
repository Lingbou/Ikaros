...
use crate::{
    patch::parse_diff_path,
    sandbox::{FileMetadata, FileSystem as ExecutionFileSystem},
};
...
impl SelfModifyStore {
    ...
    pub(super) async fn resolve_target_with_env(
        &self,
        target_path: &Path,
        ...
        let resolved =
            resolve_non_symlink_target_with_env(&workspace_root, relative, file_system).await?;
        self.validate_resolved_target(&workspace_root, &resolved)?;
        Ok(resolved)
    }

    fn validate_resolved_target(&self, workspace_root: &Path, resolved: &Path) -> Result<()> {
        if resolved.file_name().is_none() {
            return Err(IkarosError::Message(
                "self-modify target must include a file name".into(),
            ));
...
async fn resolve_non_symlink_target_with_env(
    workspace_root: &Path,
    relative: &Path,
    file_system: &dyn ExecutionFileSystem,
) -> Result<PathBuf> {
    ...
    for (index, component) in components.iter().enumerate() {
        ...
        resolved.push(name);
        let is_leaf = index + 1 == components.len();
        if let Some(metadata) = path_metadata_with_env(file_system, &resolved).await? {
...
pub(super) fn is_not_found(error: &IkarosError) -> bool {
