...
pub(in crate::chat) fn session_status_human_lines(
    ...
    let state_db = super::state_db_candidates(config, paths, workspace, runtime)?
        .into_iter()
...
#[allow(dead_code)]
pub(in crate::chat) fn print_session_history_for_human(
    config: &IkarosConfig,
    paths: &IkarosPaths,
    workspace: &Path,
    runtime: &InteractiveChatRuntime,
    ...
    for line in session_history_human_lines(config, paths, workspace, runtime, session_id, limit)? {
        println!("{line}");
    }
    Ok(())
}
...
#[allow(dead_code)]
pub(in crate::chat) fn print_session_summaries_for_human(
    config: &IkarosConfig,
    paths: &IkarosPaths,
    workspace: &Path,
    runtime: &InteractiveChatRuntime,
    ...
    for line in session_summaries_human_lines(config, paths, workspace, runtime, limit)? {
        println!("{line}");
    }
    Ok(())
}
...
fn truncate_session_text(input: &str, max_chars: usize) -> String {
