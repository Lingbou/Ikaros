...
fn init_doctor_chat_and_task_dry_run_work_with_explicit_offline_mock_config() {
    ...
    assert!(env.home.join("persona").is_dir());
    assert!(env.home.join("persona/profile.md").exists());
    assert!(env.home.join("persona/voice.md").exists());
    assert!(env.home.join("persona/relationship.md").exists());
    assert!(env.home.join("persona/work-style.md").exists());
    assert!(env.home.join("persona/safety.md").exists());
...
fn memory_projection_and_candidate_cli_manage_core_memory_surface() {
    ...
    let records = store
        .list(ikaros_state::memory::MemoryQuery {
            scope: Some("default".into()),
            include_inactive: true,
            ..ikaros_state::memory::MemoryQuery::default()
        })
    ...
    assert_eq!(
        candidate_store
            .list(ikaros_state::memory::MemoryCandidateQuery {
                status: Some(MemoryCandidateStatus::Pending),
                ..ikaros_state::memory::MemoryCandidateQuery::default()
            })
