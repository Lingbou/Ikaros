...
fn appends_and_searches_local_jsonl_memory() {
    ...
    let updated = store
        ...
        .expect("update")
        .expect("updated");
    assert!(updated.record.updated_at.is_some());
    assert!(updated.record.tags.contains(&"memory".into()));
    assert_eq!(updated.change.changed_fields, vec!["content", "tags"]);
    assert!(
        store
...
#[test]
fn memory_queries_filter_inactive_records_by_default() {
    let temp = tempfile::tempdir().expect("tempdir");
    let jsonl = JsonlMemoryStore::new(temp.path().join("jsonl"));
    assert_memory_queries_filter_inactive_records_by_default(&jsonl);

    let sqlite = SqliteMemoryStore::new(temp.path().join("sqlite"));
    assert_memory_queries_filter_inactive_records_by_default(&sqlite);
}

fn assert_memory_queries_filter_inactive_records_by_default(store: &dyn MemoryStore) {
    let mut inactive =
        MemoryRecord::new(MemoryKind::User, "default", "old preference").expect("inactive record");
    inactive.active = false;
    store.append(inactive).expect("append inactive");
    store
        .append(MemoryRecord::new(MemoryKind::User, "default", "new preference").expect("active"))
    ...
    assert_eq!(default[0].content, "new preference");
