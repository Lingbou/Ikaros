...
它会组装 repo map、change plan、可选 patch attempt、turn diff、test-matrix
evidence、review、iteration plan、loop report、final report 和可选 session replay
evidence。`code_workflow` 和 `code_edit_guarded` 里的 patch 写入都通过 session
`ExecutionEnv` filesystem interface 执行，而不是在 skill 内直接调用 host filesystem
API。`code_edit_guarded` 仍是直接应用指定 unified diff 的审批受控入口。
