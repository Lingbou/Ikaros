...
use ikaros_session::{
    AgentEvent, AgentEventKind, SessionId, SessionReplay, SessionReplayPage, SessionStore,
    SqliteSessionStore,
};
use std::{
    ...
    for state_db in &candidates {
        ...
        if !timeline_request_requires_full_replay(&request) {
                ...
                store.replay_session_page(&session_id, request.page, replay_page_size(verbosity))?
            {
                ...
                let coding_group_events = store.agent_events(&session_id)?;
                println!("{label}: found");
                ...
                print_recent_timeline_page(&replay, verbosity, &coding_group_events);
                return Ok(());
            }
            continue;
        }
...
fn print_recent_timeline_page(
    ...
    verbosity: TimelineVerbosity,
    coding_group_events: &[ikaros_session::AgentEvent],
) {
    let entry_limit = timeline_entry_limit(verbosity);
    let event_limit = timeline_event_limit(verbosity);
    ...
    print_coding_groups(coding_group_events.iter());
...
fn replay_page_size(verbosity: TimelineVerbosity) -> usize {
    timeline_entry_limit(verbosity).max(timeline_event_limit(verbosity))
}
