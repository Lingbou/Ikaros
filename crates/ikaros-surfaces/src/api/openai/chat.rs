...
pub(in crate::api) fn chat_completion_response(
    ...
    let services = api_model_services_shape_checked(
        paths,
        workspace,
        agent_override,
    ...
    let model_config = services.model_config;
    ...
    if stream_response {
        ...
        for chunk in stream.chunks.iter().filter(|chunk| !chunk.is_empty()) {
            evidence.emit(
                ...
                AgentEventKind::ModelStream(ikaros_providers::model::ModelStreamEvent::TextDelta(
