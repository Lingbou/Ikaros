...
pub(in crate::chat::workbench::screen) fn screen_input_popup_json(
    screen: &WorkbenchScreen,
    state: &WorkbenchScreenState,
) -> serde_json::Value {
    if state.command_palette_open {
        return command_palette_overlay_json(state);
    }
    ...
    if let Some(completion) = completion {
        ...
        let mut completion_items = screen
            ...
            .take(6)
            .map(input_popup_command_item_json)
            .collect::<Vec<_>>();
...
pub(in crate::chat::workbench::screen) fn command_palette_overlay_json(
    state: &WorkbenchScreenState,
) -> serde_json::Value {
    ...
    let palette_items = items
        .iter()
        .enumerate()
        .map(|(index, item)| command_palette_item_json(item, index == selected_index))
        .collect::<Vec<_>>();
...
pub(in crate::chat::workbench::screen) fn command_palette_popup_actions_json(
    open: bool,
) -> serde_json::Value {
    serde_json::json!({
        "open": open,
...
pub(in crate::chat::workbench::screen) fn command_palette_item_json(
...
pub(in crate::chat::workbench::screen) fn command_palette_primary_action(
    item: &SlashCommandPaletteItem,
) -> String {
...
pub(in crate::chat::workbench::screen) fn command_palette_kind_for_effect(
    effect: &str,
) -> &'static str {
    match effect {
...
pub(in crate::chat::workbench::screen) fn selected_palette_index(
    state: &WorkbenchScreenState,
    len: usize,
) -> usize {
    if len == 0 {
...
pub(in crate::chat::workbench::screen) fn selected_palette_command(
    state: &WorkbenchScreenState,
) -> Option<String> {
    if !state.command_palette_open {
        return None;
