...
Coding 有两条分开的 harness 路径。`code_workflow` 是 safe-read 控制面，只负责组装
repo map、change plan、候选 diff review、test evidence、iteration plan 和 final
report；它不会写文件。真正应用 patch 仍然走 `code_edit_guarded`，该路径需要审批，
并且必须满足 approval replay 规则后才能修改 workspace。审批后的 guarded patch 会
通过 session `ExecutionEnv` filesystem interface 读取、写入、创建父目录和回滚，
而不是在 skill 内直接调用 host filesystem API。
