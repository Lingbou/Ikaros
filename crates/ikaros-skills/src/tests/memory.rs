...
#[tokio::test]
async fn memory_search_can_include_pending_relationship_candidates_when_requested() {
    let temp = tempfile::tempdir().expect("tempdir");
    let workspace = temp.path().join("workspace");
    fs::create_dir_all(&workspace).expect("mkdir");
    let memory_dir = temp.path().join("memory");
    ...
    session
        .execute_skill(
            ...
            json!({
                "kind": "relationship",
                "scope": "default",
                "content": "User preference: pineapple and fengli",
                "reason": "preference_pattern",
                "tags": ["relationship", "chat-learned"]
            }),
        )
    ...
    assert_eq!(ordinary.output, json!([]));

    let with_pending = session
        .execute_read_skill_with_audit_input(
            &registry,
            "memory_search",
            json!({
        ...
        .await
        .expect("search with pending candidates");
    let records = with_pending.output.as_array().expect("records");
    assert_eq!(records.len(), 1);
    assert_eq!(
        records[0]["content"],
        json!("User preference: pineapple and fengli")
    );
