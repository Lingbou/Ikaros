...
pub(crate) fn side_panel_title(screen: &WorkbenchScreen) -> &'static str {
...
pub(crate) fn screen_modal_summary(cell: &WorkbenchCell) -> String {
    format!(
...
pub(crate) fn screen_modal_json_value(screen: &WorkbenchScreen) -> serde_json::Value {
...
pub(crate) fn approval_modal_options_json(
    approve: Option<&str>,
    deny: Option<&str>,
    inspect: &str,
...
pub(crate) fn screen_selected_json_value(
...
pub(crate) fn selected_cell_actions_json(
...
pub(crate) fn selected_replay_actions_json(commands: &[String]) -> serde_json::Value {
    serde_json::json!({
...
pub(crate) fn selected_cell_intents_json(commands: &[String]) -> Vec<serde_json::Value> {
    commands
        .iter()
        .map(|command| {
...
pub(crate) fn command_intent(command: &str) -> &'static str {
    match command_root(command) {
        ...
        "/session" | "/sessions" | "/resume" | "/new" | "/fork" => "session",
        "/context" => "context",
        "/memory" => "memory",
        "/rag" => "rag",
        "/tools" | "/mcp" => "tools",
...
pub(crate) fn command_scope(command: &str) -> &'static str {
    match command_root(command) {
...
pub(crate) fn command_target(command: &str) -> Option<String> {
    ...
    match parts.as_slice() {
        ...
        ["/session", value, ..] if !value.starts_with('-') => Some((*value).to_owned()),
        ["/screen", action, ..] => Some(action.replace('-', "_")),
        _ => None,
    }
}

pub(crate) fn command_risk(command: &str) -> &'static str {
    match command_root(command) {
        ...
        "/resume" | "/new" | "/fork" => "runtime-mutation",
        _ => "read",
    }
}

pub fn command_requires_explicit_action(command: &str) -> bool {
    let parts = command.split_whitespace().collect::<Vec<_>>();
    match parts.as_slice() {
...
pub(crate) fn command_shortcut(command: &str) -> Option<&'static str> {
    match command_root(command) {
        "/help" => Some("f1"),
        "/commands" => Some("f5"),
        "/screen" => Some("tab"),
        ...
        "/session" | "/context" | "/memory" => Some("enter"),
        _ => None,
    }
}

pub(crate) fn command_root(command: &str) -> &str {
    command.split_whitespace().next().unwrap_or("none")
}

pub(crate) fn selected_command_action(
    commands: &[String],
    prefix: &str,
...
pub(crate) fn command_with_prefix(commands: &[String], prefix: &str) -> Option<String> {
    commands
        .iter()
...
pub(crate) fn approval_id_from_decision_command(command: &str) -> Option<String> {
    command
...
pub(crate) fn command_with_prefixes(commands: &[String], prefixes: &[&str]) -> Option<String> {
    prefixes
        .iter()
        .find_map(|prefix| command_with_prefix(commands, prefix))
}

pub(crate) fn cells_json(cells: &[WorkbenchCell]) -> Vec<serde_json::Value> {
    cells
        .iter()
        .map(|cell| {
            serde_json::json!({
