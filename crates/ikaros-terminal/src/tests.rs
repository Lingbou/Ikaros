...
use crate::{WorkbenchCell, WorkbenchCellKind};
...
fn screen_navigation_marks_focus_and_applies_panel_scroll() {
    ...
    assert!(state.footer_summary().contains("focus=main"));
    assert!(state.footer_summary().contains("scroll=main:1"));
...
fn ratatui_default_snapshot_renders_clean_chat_surface_without_machine_noise() {
    ...
    assert!(snapshot.contains("U> hello"));
...
#[test]
fn human_bottom_pane_running_hint_matches_interrupt_behavior() {
    let screen = WorkbenchScreen {
        title: "Ikaros Workbench".into(),
        status: vec![
            cell(
            ...
            cell(
                ...
                "progress",
                "status=running phase=active detail=hello",
            ),
            cell(
                WorkbenchCellKind::Session,
                "bottom pane",
                "active_view=composer approvals=0 pending_inputs=0 attachments=0 continuations=0",
            ),
        ],
        timeline: vec![],
        main: vec![],
        side: vec![],
        footer: "session=session-one".into(),
        input_hint: "input_state: view=hello undo=0".into(),
    };

    let text = super::render::bottom_pane_text(&screen, &WorkbenchScreenState::default());

    assert!(text.contains("Ctrl+C interrupt"));
    assert!(text.contains("Esc cancel input"));
    assert!(!text.contains("Ctrl+C clear or exit"));
    assert!(!text.contains("Esc close panel"));
}
...
fn ratatui_default_snapshot_filters_internal_turn_notices() {
    let screen = WorkbenchScreen {
        ...
        main: vec![
            ...
            cell(
                WorkbenchCellKind::Continuation,
                "notice chat_turn",
                "notice_kind=progress detail=status=running elapsed_ms=0 detail=hello",
            ),
            cell(
    ...
    assert!(snapshot.contains("U> hello"), "snapshot:\n{snapshot}");
...
#[test]
fn ratatui_default_snapshot_renders_slash_command_input_as_plain_user_message() {
    let screen = WorkbenchScreen {
        title: "Ikaros Workbench".into(),
        status: vec![
            cell(
        ...
        main: vec![cell(
            WorkbenchCellKind::Session,
            "notice command input",
            "notice_kind=info detail=class=inspect action=inspect_model command=/model",
        )],
        side: vec![],
        footer: "session=session-one".into(),
        input_hint: "input_state: view= undo=0".into(),
    ...
    let snapshot =
        render_tui_workbench_snapshot(&screen, &state, 96, 16).expect("ratatui snapshot");

    assert!(snapshot.contains("*  /model"), "snapshot:\n{snapshot}");
    assert!(!snapshot.contains("[Inspect model]"));
    assert!(!snapshot.contains("action=inspect_model"));
    assert!(!snapshot.contains("notice_kind="));
}
...
fn ratatui_default_snapshot_preserves_assistant_markdown_structure() {
    ...
    assert!(
        snapshot.contains("*  Browse and edit workspace files"),
        "snapshot:\n{snapshot}"
    );
    assert!(
        snapshot.contains("*  Review Git changes"),
        "snapshot:\n{snapshot}"
    );
...
fn ratatui_default_snapshot_separates_chat_turns() {
    ...
    assert!(snapshot.contains("U> first"));
    assert!(snapshot.contains("U> second"));
...
#[test]
fn fullscreen_frame_keeps_selected_primary_action_out_of_input_footer() {
    let screen = WorkbenchScreen {
        title: "Ikaros Workbench".into(),
        status: vec![],
        timeline: vec![],
    ...
    assert!(frame.contains("action=/cancel cont-one"));
...
fn screen_navigation_parses_workbench_screen_arguments() {
    ...
    assert!(state.footer_summary().contains("scroll=main:3"));
}
...
fn screen_command_palette_enter_is_noop_without_matches() {
    ...
    let popup = command_palette_overlay_json(&screen, &state);
