...
Running `ikaros` with no subcommand, `ikaros tui`, or `ikaros chat` without
`--message` starts the terminal workbench. `ikaros workbench` is the explicit
name for the same surface. It keeps the one-shot CLI commands available for
scripting, but gives the normal interactive path history, multiline input,
cursor-aware line editing, undo for the in-memory input buffer, mouse-wheel
history scrolling, bracketed paste, queued input, default streaming,
session resume,
...
When the workbench is running as a real fullscreen TTY, read-only/status slash
commands are routed back into the screen instead of printing raw stdout. This
includes the help and command-list paths, timeline/replay/debug/trace views,
runtime status views, provider/model inspection, MCP/API/gateway/task status,
and local browser/web/vision/image help surfaces. `/help` and `/commands`
open the command palette directly. Non-TTY runs and explicit screen/debug
commands keep their deterministic protocol output for scripts.
...
cancel state.
`/screen` renders a deterministic workbench frame. `ikaros tui` starts with the
same screen model in a persistent alternate-screen session and restores the
terminal when the REPL exits. On a real TTY, persistent fullscreen refreshes draw
through ratatui's `CrosstermBackend`, enables bracketed paste and mouse capture,
and supports mouse-wheel history scrolling; non-TTY runs keep the deterministic
snapshot/protocol output used by scripts and smoke tests. After each interactive
turn and slash command, the TUI redraws the fullscreen frame so timeline,
provider, context, approval, queue, and live-cell state stay visible without a
manual `/screen` refresh. While a streamed turn is running, the cached screen
also inserts the submitted user message as `user turn=pending` before model
deltas arrive, so the user sees the active input immediately. The `--fullscreen`
flag wraps a single refresh in an alternate-screen terminal envelope with cursor
...
The fullscreen footer mirrors the selected cell's primary action as
`enter=<command>`, so the active Enter target is visible in the frame itself.
...
bracketed paste is enabled in fullscreen input mode. Control actions emit an
...
raw-mode input, mouse-wheel scrolling, real-TTY fullscreen drawing, and
deterministic non-TTY snapshots, but it is still not a complete async TUI
application.
