...
不传子命令的 `ikaros`、`ikaros tui`，以及不传 `--message` 的 `ikaros chat` 都会进入
terminal workbench。`ikaros workbench` 是同一个交互面的显式命令。一次性 CLI
仍保留给脚本和测试；普通交互路径提供 history、multiline input、cursor-aware
line editing、内存输入缓冲区 undo、鼠标滚轮滚动、bracketed paste、pending input
queue、默认 streaming、session resume、timeline/replay/debug、provider/gateway/task
...
`/provider inspect`。
workbench 在真实 fullscreen TTY 中运行时，只读/status slash command 会路由回
screen，不再打印 raw stdout。这个范围包括 help 和 command list、timeline/replay/
debug/trace、runtime status、provider/model inspect、MCP/API/gateway/task status，
以及本地 browser/web/vision/image help surface。`/help` 和 `/commands` 会直接打开
command palette。非 TTY 运行和显式 screen/debug 命令仍保留脚本使用的确定性
protocol 输出。
...
`/screen` 会渲染确定性的 workbench frame。`ikaros tui` 会用同一套 screen model 启动持久
alternate-screen session，并在 REPL 退出时恢复终端。真实 TTY 下，persistent fullscreen
刷新会通过 ratatui 的 `CrosstermBackend` 直接绘制，启用 bracketed paste 和 mouse
capture，并支持鼠标滚轮滚动历史；非 TTY 运行仍保留脚本和 smoke test 使用的确定性
snapshot/protocol 输出。每个交互 turn 和 slash command 结束后，TUI 会自动重绘
fullscreen frame，让 timeline、provider、context、approval、queue 和 live-cell 状态不需要
手动 `/screen` 也能保持可见。streamed turn 运行中，cached screen 会先把刚提交的用户输入
插成 `user turn=pending`，模型 delta 还没到时也能立刻看到当前输入。`--fullscreen`
会把单次刷新包进 alternate-screen terminal envelope，并包含 hide/show cursor、
clear/home 控制序列；`--inline` 会回到适合脚本读取的行式 frame。`--focus`、`--scroll` 和 `--select` 是
timeline/main/side panel 的第一版可导航 screen slice；连续 `/screen` 命令会保留当前
workbench session 的 focus、scroll 状态和每个 panel 独立的 selected row。每次刷新都会输出
`screen_selected`，包含当前 focused cell 的 panel、row、kind、title 和 detail，方便脚本和
replay view 不解析 frame 也能查看选中内容；还会输出 `screen_selected_actions`，当选中的
cell 有足够证据时直接给出 timeline、trace、debug、approval、cancellation 和 queue 后续命令；
...
fullscreen footer 会把当前选中 cell 的 primary action 显示成 `enter=<command>`，因此用户不用读
JSON 也能知道回车会执行什么。
...
readline 风格的 Ctrl-P/Ctrl-N/Ctrl-B/Ctrl-F/Ctrl-D 快捷键。Ctrl-U 和 Ctrl-K 会删除光标前后内容，
Shift+Enter 或 Alt+Enter 会插入换行，fullscreen input mode 会启用 bracketed paste。控制动作会输出
`input_state` 行，包含 cursor 位置、脱敏后的 buffer、cursor view 和 slash completion
candidates；这是 ratatui/crossterm 支撑的第一版，有 raw-mode input、鼠标滚轮滚动、
真实 TTY fullscreen 绘制和确定性非 TTY snapshot，但还不是完整 async TUI 应用。
