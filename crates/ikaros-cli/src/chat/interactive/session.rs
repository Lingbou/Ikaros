...
use ikaros_agent::chat::{
    ChatRunOptions,
    workbench_state::{WorkbenchForkReport, fork_workbench_session},
};
...
pub(super) fn handle_fork_command(
    ...
    match fork_workbench_session(
        ...
        WorkbenchForkReport::SessionNotFound { session_id } => {
        ...
        WorkbenchForkReport::MissingActiveLeaf { session_id } => {
            if runtime.fullscreen_stdout_quiet() {
                runtime.push_notice(WorkbenchNotice::new(
                    WorkbenchNoticeKind::Error,
                    "session fork",
