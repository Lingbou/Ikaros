...
消息网关是外部消息使用的本地 inbox/outbox 和 adapter 边界。消息进入时不会执行工作；
worker/drain 再通过 runtime 和 harness 路径处理。稳定的跨界 wire shape 属于
`ikaros-protocol`；`ikaros-gateway` 拥有 queue state、adapter mapping、webhook
ingress、admin operation 和 gateway-local policy check。
...
Gateway-local queue record 和 adapter policy 放在 `ikaros-gateway`。需要被 API、
TUI、replay 和后续 adapter 共享的稳定跨界 wire shape 属于 `ikaros-protocol`。
重构期间，如果 `ikaros-gateway` 里仍有同名 frame type，在迁移或投影进 protocol
crate 前，它们只是 gateway implementation detail。
...
- 稳定的 gateway/session wire shape 位于 `ikaros-protocol`；adapter 应在这些 shape
  暴露后依赖它们，并在迁移完成前把 `ikaros-gateway` 的 frame/store type 当作
  gateway-local。Adapter 仍应面向 inbox/outbox queue，而不是 runtime internal。
