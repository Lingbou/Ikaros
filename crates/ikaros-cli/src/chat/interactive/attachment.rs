    ...
    match args.as_slice() {
        ...
        ["clear"] => {
            ...
            if quiet {
                runtime.push_notice(WorkbenchNotice::info(
                    "attachments",
                    &format!("cleared={cleared} pending=0"),
                ));
            } else if human {
            ...
            if index == 0 || index > runtime.pending_content_blocks.len() {
                if quiet {
                    runtime.push_notice(WorkbenchNotice::error(
                        "attachment remove",
                        &format!(
                            "index={} reason=not_found pending={}",
                            index,
                            runtime.pending_content_blocks.len()
                        ),
                    ));
                    ...
                    println!("* Attachments");
                    println!("  attachment not found: {index}");
                    println!("  pending: {}", runtime.pending_content_blocks.len());
                } else {
                    println!(
                ...
                let kind = content_block_kind(&removed);
                if quiet {
                    runtime.push_notice(WorkbenchNotice::info(
                        "attachment removed",
                        &format!(
                            "index={} kind={} remaining={}",
                            index,
                            kind,
                            runtime.pending_content_blocks.len()
                        ),
                    ...
                    println!("  index: {index}");
                    println!("  kind: {kind}");
                    println!("  remaining: {}", runtime.pending_content_blocks.len());
                } else {
                    println!(
        ...
        ["image", rest @ ..] => {
            ...
            if let Some(detail) = detail {
                if let ikaros_providers::model::ModelContentBlock::Image {
                    detail: image_detail,
                    ..
                } = &mut block
                {
            ...
            if quiet {
                runtime.push_notice(WorkbenchNotice::info(
                    "attachment queued",
                    &format!(
                        "kind={} pending={} attachments_force_single_call=true",
                        kind,
                        runtime.pending_content_blocks.len()
                    ),
                ));
            } else if human {
                println!("* Attachment queued");
                println!("  kind: {kind}");
                println!("  pending: {}", runtime.pending_content_blocks.len());
            } else {
                println!(
...
fn emit_attachment_usage(runtime: &mut InteractiveChatRuntime) {
    if runtime.fullscreen_stdout_quiet() {
        runtime.push_notice(WorkbenchNotice::info(
            "attachments",
            "usage is available in the fullscreen attachments cell",
        ));
        ...
        println!("  usage: /attach list | /attach remove <index> | /attach clear");
    } else {
...
fn print_attachment_status_for_human(runtime: &InteractiveChatRuntime) {
    println!("* Attachments");
    println!("  pending: {}", runtime.pending_content_blocks.len());
    println!(
        "  next turn: {}",
    ...
    for (index, block) in runtime.pending_content_blocks.iter().take(5).enumerate() {
        println!(
            "  {}. {}",
            index + 1,
            terminal_inline(&content_block_summary(block))
