...
pub(in crate::chat) enum WorkbenchInputAction {
    HistoryPrevious,
    HistoryNext,
    #[allow(dead_code)]
    HistorySearchStart,
...
pub(in crate::chat) enum WorkbenchInputEvent {
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub(in crate::chat) enum WorkbenchTerminalInputEvent {
    Action(WorkbenchInputAction),
    InsertText(String),
    InsertNewline,
    Submit,
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub(in crate::chat) enum WorkbenchTerminalInputOutcome {
    Pending,
    Submit(String),
    Exit,
}

pub(in crate::chat) fn apply_workbench_terminal_input_event(
...
pub(in crate::chat) fn parse_workbench_terminal_key_event(
    event: KeyEvent,
    ...
    match event.code {
        ...
        KeyCode::Char('l') if control => Some(WorkbenchTerminalInputEvent::Action(
            WorkbenchInputAction::Refresh,
        )),
...
pub(in crate::chat) fn parse_workbench_terminal_event(
    ...
    match event {
        ...
        CrosstermEvent::Paste(text) => Some(WorkbenchTerminalInputEvent::InsertText(
            terminal_inline(&text),
        )),
        _ => None,
    }
}

pub(in crate::chat) fn parse_workbench_input_event(input: &str) -> WorkbenchInputEvent {
...
pub(in crate::chat) fn format_workbench_input_state(
    action: &str,
...
pub(in crate::chat) fn format_workbench_input_model_detail(
    action: &str,
    state: &WorkbenchInputState,
) -> String {
    let completion_candidates = state.completion_candidates();
    let completion_active = state.completion_cycle.is_some();
    let completion_selected = state.completion_selected().unwrap_or("none");
    let history_search_active = state.history_search_active();
    let history_cursor = state
        .history_cursor
        .map(|cursor| cursor.saturating_add(1).to_string())
        .unwrap_or_else(|| "none".into());
    format!(
        "action={} mode={} cursor={} chars={} lines={} empty={} dirty={} history_entries={} history_cursor={} undo={} redo={} completion_active={} completion_query={} completion_selected={} completion_candidates={} history_search_active={} history_search_summary={} buffer={} cursor_view={} submit=enter newline=alt-enter cancel=esc interrupt=ctrl-c:exit_or_clear_or_interrupt complete=tab history=ctrl-r undo_key=ctrl-z redo_key=ctrl-y word_left=alt-b word_right=alt-f delete_word=ctrl-w/alt-d",
        terminal_inline(action),
        if history_search_active {
            "history_search"
        } else if state.buffer.trim_start().starts_with('/') {
            "slash_command"
        } else {
            "message"
        },
        ...
        state.buffer.lines().count().max(1),
        ...
        state.redo_stack.len(),
        completion_active,
        terminal_inline(&state.completion_query()),
        terminal_inline(completion_selected),
        if completion_candidates.is_empty() {
            "none".into()
        } else {
            terminal_inline(&completion_candidates.join(","))
        },
...
pub(in crate::chat) struct WorkbenchInputState {
    buffer: String,
    cursor: usize,
    history: Vec<String>,
...
impl WorkbenchInputState {
    pub(in crate::chat) fn from_history(entries: impl IntoIterator<Item = String>) -> Self {
    ...
    pub(in crate::chat) fn buffer(&self) -> &str {
        &self.buffer
    }

    pub(in crate::chat) fn cursor(&self) -> usize {
        self.cursor
    }

    pub(in crate::chat) fn cursor_view(&self) -> String {
        input_cursor_view(&self.buffer, self.cursor)
    }

    pub(in crate::chat) fn buffer_is_empty(&self) -> bool {
        self.buffer.trim().is_empty()
    }

    pub(in crate::chat) fn set_buffer(&mut self, input: &str) {
    ...
    pub(in crate::chat) fn insert_text(&mut self, input: &str) {
    ...
    pub(in crate::chat) fn insert_newline(&mut self) {
        self.push_undo();
        self.clear_completion_cycle();
        self.clear_history_search();
        let byte_index = self.byte_index_for_cursor(self.cursor);
    ...
    pub(in crate::chat) fn record_history(&mut self, input: &str) {
    ...
    pub(in crate::chat) fn history_search_active(&self) -> bool {
        self.history_search.is_some()
    }

    pub(in crate::chat) fn history_search_summary(&self) -> String {
    ...
    pub(in crate::chat) fn history_search_candidates(&self, limit: usize) -> Vec<String> {
        let Some(search) = &self.history_search else {
            return Vec::new();
        };
        search
    ...
    pub(in crate::chat) fn insert_history_search_text(&mut self, input: &str) {
    ...
    pub(in crate::chat) fn accept_history_search(&mut self) {
    ...
    pub(in crate::chat) fn cancel_history_search(&mut self) {
    ...
    pub(in crate::chat) fn cancel_transient_input(&mut self) -> bool {
    ...
    #[cfg(test)]
    pub(super) fn history_entries(&self) -> &[String] {
        &self.history
    }

    pub(in crate::chat) fn completion_candidates(&self) -> Vec<String> {
    ...
    pub(in crate::chat) fn completion_query(&self) -> String {
    ...
    pub(in crate::chat) fn completion_active_summary(&self) -> String {
    ...
    pub(in crate::chat) fn completion_selected(&self) -> Option<&str> {
        let cycle = self.completion_cycle.as_ref()?;
        cycle.candidates.get(cycle.index).map(String::as_str)
    }

    pub(in crate::chat) fn apply(&mut self, action: WorkbenchInputAction) -> Option<String> {
    ...
    fn complete(&mut self) -> Option<String> {
        ...
        let existing_cycle = self.completion_cycle.as_ref().and_then(|cycle| {
            cycle
                .candidates
                .get(cycle.index)
                .is_some_and(|selected| selected == &current_token)
                .then(|| cycle.clone())
        });
        let (query, candidates, index, append_space) = if let Some(mut cycle) = existing_cycle {
            cycle.index = (cycle.index + 1) % cycle.candidates.len();
            (cycle.query, cycle.candidates, cycle.index, false)
        } else {
            let candidates = slash_command_completion_candidates(&query, 24)
                .into_iter()
                .map(|candidate| candidate.name.to_owned())
                .collect::<Vec<_>>();
            if candidates.is_empty() {
            ...
            (query, candidates.clone(), 0, candidates.len() == 1)
        };
        let selected = candidates.get(index)?.clone();
        self.push_undo();
        self.replace_completion_token(&selected, append_space);
        self.completion_cycle = (!append_space).then_some(WorkbenchCompletionCycle {
            query,
            candidates,
            index,
        });
