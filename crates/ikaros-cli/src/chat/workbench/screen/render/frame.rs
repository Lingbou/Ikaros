...
pub(in crate::chat) fn render_fullscreen_workbench_with_state(
    ...
    if state.command_palette_open {
        lines.push(framed_line(&command_palette_summary_line(state), width));
    }
    ...
    lines.push(framed_line(&state.footer_summary(), width));
    lines.push(separator(width));
...
pub(in crate::chat) fn draw_tui_workbench_frame(
    frame: &mut Frame<'_>,
    screen: &WorkbenchScreen,
    state: &WorkbenchScreenState,
) {
    ...
    draw_command_palette_overlay(frame, state);
    draw_approval_modal_overlay(frame, screen);
}

pub(in crate::chat::workbench::screen) fn draw_chat_tui_frame(
    frame: &mut Frame<'_>,
    screen: &WorkbenchScreen,
    state: &WorkbenchScreenState,
) {
