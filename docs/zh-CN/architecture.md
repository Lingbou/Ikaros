...
- `MemoryScore`、`MemoryPolicy` 和 `MemoryJournal` 属于 `ikaros-memory`。Runtime chat 会把 `sync_turn` append 和 skipped-write 决策写入 journal。Promotion、demotion、forget 和 quota decision 成为 runtime 行为时，也应使用同一边界。
