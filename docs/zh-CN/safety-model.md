...
Ikaros 把工具使用视为受控操作。`ikaros-toolkit` 定义共享工具契约，
`ikaros-sandbox` 提供具体本地执行 backend，harness 判断模型、persona、命令或
调度器是否可以使用这些能力。
...
模型、RAG embedding 和语音 provider 都通过 adapter 访问。当前实现支持的路径会在
provider 调用前脱敏。用量日志只存 provider、model、token 等元数据，不存 prompt。

Provider key 和 base URL 从本地 `IKAROS_HOME/config.yaml` 读取。当前模型通常使用
`model.default`，共享资源池仍使用 `providers.*`。不要把 key 写入仓库文件、审计日志、
记忆或 RAG 索引。
...
命令型插件只通过内置 plugin runner skill 执行。插件 manifest 声明 risk 和
command 元数据，但运行时仍会做策略评估。插件 stdin/stdout/stderr 会在 harness
输出中脱敏。
...
它需要已存储的 proposal、rollback snapshot、明确 approval id、drift check 和受限
