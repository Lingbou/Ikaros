...
fn session_search_uses_fts_trigram_and_substring_indexes() {
    ...
    let chinese = sample_entry(
        ...
        turn_id.clone(),
        SessionEntryKind::AssistantMessage,
        "\u{4e2d}\u{6587}\u{641c}\u{7d22}\u{4f53}\u{9a8c}\u{9700}\u{8981}\u{652f}\u{6301}\u{5b50}\u{4e32}\u{5339}\u{914d}\u{3002}",
    );
    let other = sample_entry(
    ...
    let trigram_hits = store
        .search_entries(
            &SessionSearchQuery::new("\u{641c}\u{7d22}\u{4f53}\u{9a8c}")
                .for_session(session_id.clone())
                .with_limit(10),
        )
    ...
    assert!(
        trigram_hits[0]
            .snippet
            .contains("\u{641c}\u{7d22}\u{4f53}\u{9a8c}")
    );

    let short_cjk_hits = store
        .search_entries(
            &SessionSearchQuery::new("\u{4e2d}\u{6587}")
