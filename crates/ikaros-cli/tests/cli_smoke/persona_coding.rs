...
fn assert_default_entry_has_no_protocol_fragments(output: &str) {
    for fragment in [
        "screen_json:",
        "workbench_evidence",
        "_json:",
        ...
        assert!(
            ...
            "default entry leaked protocol fragment {fragment}: {output}"
        );
    }
}

fn assert_terminal_output_contains_unwrapped(output: &str, expected: &str) {
    let unwrapped = output.replace("\r\n", "\n").replace("\n  ", "");
    assert!(
        unwrapped.contains(expected),
        "terminal output did not contain {expected:?}\nraw output:\n{output}\nunwrapped output:\n{unwrapped}"
    );
}
...
fn persona_and_relationship_paths_are_local_audited_and_searchable() {
    ...
    let persona_md =
        fs::read_to_string(env.home.join("persona/profile.md")).expect("persona markdown");
...
fn chat_workbench_screen_footer_and_paged_timeline_are_navigable() {
    ...
    assert!(!output.contains("focus=side approval_action="));
...
fn chat_workbench_exposes_session_aliases_model_context_memory_rag_diff_and_timeline() {
    ...
    assert!(output.contains("provider: mock"));
    assert!(output.contains("session_cleared:"));
...
#[test]
fn default_entry_opens_quiet_human_chat_without_machine_readable_screen_noise() {
    let env = TestHome::new();
    env.init();
    env.use_offline_mock_config();
    install_smoke_rust_crate(&env.workspace);
    ...
    let screen = env.run_with_stdin(
        ["--chat-session", "default-screen-session"],
        "/screen --raw --focus side --select 1\n/quit\n",
    );
    assert!(screen.contains("screen_mode: refreshed"));
    assert!(screen.contains("screen_json:"));
    assert!(screen.contains("\"schema\":\"ikaros-workbench-screen-v1\""));
}
...
fn default_entry_chat_turn_does_not_emit_screen_debug_snapshot() {
    ...
    assert_terminal_output_contains_unwrapped(&output, "refresh the fullscreen timeline");
    assert!(output.contains("Mock Ikaros plan"));
    assert!(!output.contains("chat_turn:"));
    assert!(!output.contains("workbench_progress:"));
    assert!(!output.contains("workbench_progress_json:"));
    assert!(!output.contains("Working ("));
    assert!(!output.contains("elapsed_ms=0"));
    assert!(!output.contains("screen_mode:"));
    assert!(!output.contains("screen_json:"));
    assert!(!output.contains("\"schema\":\"ikaros-workbench-screen-v1\""));

    let history = env.run(["chat", "--history"]);
    assert!(history.contains("refresh the fullscreen timeline"));
    assert!(history.contains("streamed=true"));
}
...
fn default_entry_slash_command_does_not_emit_screen_debug_snapshot() {
    ...
    assert!(!output.contains("pending_input_queued:"));
    assert!(!output.contains("screen_mode:"));
    assert!(!output.contains("screen_json:"));
    assert!(!output.contains("\"schema\":\"ikaros-workbench-screen-v1\""));
    assert!(!output.contains("sk-secret-value"));
...
#[test]
fn default_entry_model_command_renders_human_status_without_protocol_lines() {
    let env = TestHome::new();
    env.init();
    env.use_offline_mock_config();
    install_smoke_rust_crate(&env.workspace);

    let output = env.run_with_stdin(std::iter::empty::<&str>(), "/model\n/quit\n");

    assert!(output.contains("* Model"));
    assert!(output.contains("  provider: mock"));
    assert!(!output.contains("model_source:"));
    assert!(!output.contains("configured_profile:"));
...
fn default_entry_help_and_unknown_command_render_human_blocks() {
    ...
    assert!(output.contains("suggestion: /sessions"));
    assert!(output.contains("* Command failed."));
    assert!(!output.contains("unknown command:"));
    assert!(!output.contains("did_you_mean:"));
    assert!(!output.contains("commands: /"));
    assert!(!output.contains("interactive_command_error_json:"));
    assert_default_entry_has_no_protocol_fragments(&output);
}
...
fn default_entry_plain_text_input_keeps_first_letter_when_it_matches_screen_shortcut() {
    ...
    assert_terminal_output_contains_unwrapped(&output, message);
    assert!(output.contains("Mock Ikaros plan"));
    assert!(!output.contains("chat_turn:"));
...
#[test]
fn default_entry_input_control_sequences_do_not_emit_protocol_lines() {
    let env = TestHome::new();
    env.init();
    env.use_offline_mock_config();
    install_smoke_rust_crate(&env.workspace);
    ...
    assert!(!output.contains("input_history_selected:"));
    assert!(!output.contains("input_completion:"));
    assert!(!output.contains("input_edit:"));
    assert!(!output.contains("input_state:"));
    assert_default_entry_has_no_protocol_fragments(&output);
}
...
fn chat_workbench_streaming_turn_prints_rendered_markdown_transcript() {
    ...
    assert!(output.contains("--- diff"));
    assert!(output.contains("| -old [REDACTED_SECRET]"));
...
fn chat_workbench_context_and_memory_show_timeline_cells() {
    ...
    assert!(output.contains("memory_working_active: 2"));
    ...
    assert!(output.contains("\"working_active\":2"));
