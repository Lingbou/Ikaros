...
use ikaros_state::session::{SessionEntry, SessionEntryKind, SessionReplay, SessionStore};
use serde_json::json;
...
mod tests {
    ...
    #[test]
    fn delete_tombstone_hides_session_replay_from_chat_history_projection() {
        let temp = tempfile::tempdir().expect("tempdir");
        let store = SqliteSessionStore::new(temp.path());
        let session_id = SessionId::from("history-delete-session");
        let mut user = SessionEntry::new(session_id.clone(), SessionEntryKind::UserMessage);
        user.visible_text = Some("remember this".to_owned());
        store.append_entry(&user).expect("append user");
        let replay = store
            .replay_session(&session_id)
            .expect("read replay")
            .expect("session replay");
        assert!(!session_replay_hides_chat_history(&replay));

        append_chat_history_delete_tombstone(&store, &replay, "history_delete_session")
            .expect("append tombstone");

        let replay = store
            .replay_session(&session_id)
            .expect("read hidden replay")
            .expect("hidden session replay");
        assert!(session_replay_hides_chat_history(&replay));
        let tombstone = replay
            .entries
            .iter()
            .find(|entry| {
                entry.payload["operation"].as_str() == Some(CHAT_HISTORY_DELETE_SESSION_OPERATION)
            })
            .expect("delete tombstone entry");
        assert_eq!(tombstone.kind, SessionEntryKind::Custom);
        assert_eq!(tombstone.parent_entry_id.as_ref(), Some(&user.entry_id));
        assert_eq!(tombstone.payload["reason"], "history_delete_session");
    }
}
