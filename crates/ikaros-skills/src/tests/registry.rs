...
#[test]
fn builtin_registry_builder_composes_selected_skill_groups() {
    let temp = tempfile::tempdir().expect("tempdir");
    let workspace = temp.path().join("workspace");
    fs::create_dir_all(&workspace).expect("mkdir");
    let registry = BuiltinRegistryBuilder::empty(test_env(temp.path(), &workspace))
        .with_groups([
            BuiltinSkillGroup::Core,
            BuiltinSkillGroup::Rag,
            BuiltinSkillGroup::ToolBridge,
        ])
        .build();
    let names = registry.names();

    assert!(names.contains(&"task_summarize".to_string()));
    ...
    assert!(!names.contains(&"memory_append".to_string()));
    assert!(!names.contains(&"plugin_command_run".to_string()));

    let visible =
        registry.model_visible_names_for(&ToolsetSelection::new([Toolset::Core, Toolset::Rag]));
    assert!(visible.contains(&"tool_search".to_string()));
    assert!(!visible.contains(&"rag_search".to_string()));
}
