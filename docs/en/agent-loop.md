...
The agent loop is the model-guided execution path in `ikaros-agent`. It lets a
...
provider wire format, policy decisions, or host execution. Those
responsibilities belong to `ikaros-providers`, `ikaros-execution`, and
dependencies assembled by `ikaros-host`.
...
- `SkillRegistry`: executable tools from `ikaros-execution::toolkit`, governed through the
  harness.
- `AgentLoopOptions`: iteration, sampling, streaming, and guardrail settings.

The default implementation is `HarnessAgentRuntime` in `ikaros-agent`. Callers
that need a different loop implementation should swap the agent runtime value,
...
`SessionStore`. `ikaros-state::session` stores queued, running, completed, failed, and
...
`AgentEventSink`. `ikaros-state::session` provides `PersistingAgentEventSink` for
