...
pub(in crate::api) fn audio_speech_response(
    ...
    let provider_response = tokio::task::block_in_place(|| {
        handle.block_on(send_provider_json_request_for_instance(
            paths,
            &config,
            &agent,
...
pub(in crate::api) fn audio_transcription_response(
    ...
    let handle = tokio::runtime::Handle::current();
    let provider_response = tokio::task::block_in_place(|| {
        handle.block_on(send_provider_bytes_request_for_instance(
            paths,
            &config,
            &agent,
