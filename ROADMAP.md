...
caller.
The new `ikaros-host`, `ikaros-toolkit`, and `ikaros-sandbox` crates follow that
rule. Host owns runtime assembly; tools owns reusable skill/tool contracts;
sandbox owns concrete execution backends and governed network adapters.
...
- `ikaros-toolkit` centralizes reusable skill/tool contracts, execution
  interfaces, toolset metadata, and scheduling descriptors.
- `ikaros-sandbox` centralizes workspace filesystem scope, process runners,
  dry-run/Docker backends, governed network egress, and sandbox diagnostics.
