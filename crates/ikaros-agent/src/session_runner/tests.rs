...
use ikaros_execution::harness::{
    ExecutionSession, Skill, SkillContext, SkillOutput, SkillRegistry,
};
use ikaros_providers::model::{
    MockModelProvider, ModelProvider, ModelRequest, ModelResponse, TokenUsage,
};
...
#[tokio::test]
async fn session_runner_runs_turn_with_stable_session_and_event_first_result() {
...
#[tokio::test]
async fn session_runner_continue_drains_queued_messages_by_priority() {
    let runtime = RecordingRuntime::default();
    let provider = MockModelProvider::default();
    let session = test_session();
    let registry = SkillRegistry::new();
...
#[tokio::test]
async fn session_runner_can_use_caller_supplied_turn_id() {
    let runtime = RecordingRuntime::default();
    let provider = MockModelProvider::default();
    let session = test_session();
    let registry = SkillRegistry::new();
...
#[tokio::test]
async fn session_runner_uses_caller_supplied_turn_id_once() {
    let runtime = RecordingRuntime::default();
    let provider = MockModelProvider::default();
    let session = test_session();
    let registry = SkillRegistry::new();
...
#[tokio::test]
async fn session_runner_persists_continuations_across_harness_instances() {
...
#[tokio::test]
async fn session_runner_runs_persisted_continuation_with_turn_sink_without_locking() {
    let temp = tempfile::tempdir().expect("tempdir");
    let store = SqliteSessionStore::new(temp.path().join("state"));
...
#[test]
fn session_runner_cancels_durable_continuation_with_replay_event() {
    let temp = tempfile::tempdir().expect("tempdir");
    let store = SqliteSessionStore::new(temp.path().join("state"));
...
#[tokio::test]
async fn session_runner_returns_emitted_events_as_report_source() {
...
#[tokio::test]
async fn session_runner_cancel_aborts_next_turn_before_provider_request() {
    let runtime = HarnessAgentRuntime;
...
#[test]
fn session_runner_phase_guard_resets_phase_during_unwind() {
...
#[test]
fn session_runner_phase_operations_append_session_tree_entries() {
    let temp = tempfile::tempdir().expect("tempdir");
    let store = SqliteSessionStore::new(temp.path().join("state"));
    let runtime = RecordingRuntime::default();
    let provider = MockModelProvider::default();
...
#[tokio::test]
async fn session_runner_runs_retry_and_compaction_as_durable_continuations() {
    let temp = tempfile::tempdir().expect("tempdir");
    let store = SqliteSessionStore::new(temp.path().join("state"));
    let runtime = RecordingRuntime::default();
    let provider = MockModelProvider::default();
...
#[tokio::test]
async fn session_runner_runs_tool_result_continuation_with_tool_output() {
    let temp = tempfile::tempdir().expect("tempdir");
    let store = SqliteSessionStore::new(temp.path().join("state"));
    let runtime = RecordingRuntime::default();
    let provider = MockModelProvider::default();
...
#[tokio::test]
async fn session_runner_enqueues_tool_result_continuation_for_recoverable_tool_report() {
    let temp = tempfile::tempdir().expect("tempdir");
    let store = SqliteSessionStore::new(temp.path().join("state"));
