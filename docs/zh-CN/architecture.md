...
- `ikaros-context`：context bundle、section、reference、token budget、启发式 token 估算和 context diff。
...
- Context primitive 属于 `ikaros-context`。Runtime chat 会把 relationship、显式 reference、history、memory 和 RAG 组装成 token-budgeted `ContextBundle`。
