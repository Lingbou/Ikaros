...
pub(in crate::chat) fn print_workbench_command_error(
    ...
    println!(
        "workbench_command: failed command={} error={}",
        redact_secrets(command),
        redact_secrets(&message)
    );
    println!("{}", workbench_command_error_json_line(input, error));
    println!(
        "workbench_command_recovery_hint: use /help or /commands to inspect available commands"
    );
}

pub(in crate::chat) fn workbench_command_error_json_line(
    input: &str,
    error: &anyhow::Error,
) -> String {
    let command = input.split_whitespace().next().unwrap_or(input);
    ...
    let payload = json!({
        "schema": "ikaros-workbench-command-error-v1",
        "version": 1,
        ...
        "recoverable": true,
        "actions": workbench_command_error_actions(command, error_kind),
    });
    let encoded = serde_json::to_string(&payload).unwrap_or_else(|_| {
        r#"{"schema":"ikaros-workbench-command-error-v1","version":1,"status":"failed","error_kind":"unknown","message":"failed to serialize error","recoverable":true,"actions":[]}"#
            .to_owned()
    });
    format!("workbench_command_error_json: {encoded}")
}

fn workbench_command_error_actions(command: &str, error_kind: &str) -> serde_json::Value {
    let mut actions = vec![
        json!({
            "label": "help",
            "command": "/help",
            "description": "Show the workbench command list",
        }),
        json!({
            "label": "commands",
            "command": "/commands",
