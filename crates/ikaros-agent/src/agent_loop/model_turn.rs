...
use ikaros_execution::harness::{AuditEvent, ExecutionSession, GuardrailState, SkillRegistry};
use ikaros_protocol::context::HeuristicTokenEstimator;
use ikaros_providers::model::{
    ModelMessage, ModelProvider, ModelRequest, ModelStreamEvent, ModelStreamEventSink,
    ProviderRegistry,
};
use ikaros_state::session::{SessionId, TurnId};
use serde_json::json;
            ...
            if options.cancellation.is_cancelled() {
                state.tool_results.extend(emit_cancelled_tool_call_events(
                    ToolBatchDispatchContext {
                        session,
                        registry,
                        direct_manifest: &tool_definitions,
