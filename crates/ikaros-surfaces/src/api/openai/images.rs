...
pub(in crate::api) fn image_generation_response(
    ...
    let provider_response = match tokio::task::block_in_place(|| {
        handle.block_on(send_model_json_request_for_instance(
            paths,
            &config,
            &agent,
            &provider,
        ...
        Err(error) if error.to_string().contains("providers.model.base_url") => {
            return Ok(ApiHttpResponse::json_error(
                400,
                "Bad Request",
                error.to_string(),
        ...
        Err(error) => {
            return Err(error).with_context(|| "image generation provider request failed");
        }
    };
