...
"#
    ...
    let registry = builtin_registry(test_env(temp.path(), workspace));
    let session = ExecutionSession::new(workspace, temp.path().join("audit"));
    let oversized = "x".repeat(ikaros_execution::harness::PLUGIN_COMMAND_MAX_STDIN_BYTES + 1);
...
"#
    ...
    assert!(
        error_text
            .contains(&ikaros_execution::harness::PLUGIN_COMMAND_MAX_OUTPUT_BYTES.to_string())
    );
}
