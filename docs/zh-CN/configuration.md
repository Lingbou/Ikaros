...
`ikaros init` 会创建 runtime home，并只写一个配置文件：`IKAROS_HOME/config.toml`。它不会从仓库里的 example 目录读取配置。

## Provider 设置

不要把真实 API key 放进文档、测试、示例或 tracked 文件。本地未跟踪的 `IKAROS_HOME/config.toml` 可以直接保存第三方 API key，用于普通运行和 smoke test。
