...
//! Test command policy and failure analysis for governed execution paths.
...
mod tests {
    ...
    #[test]
    fn analyzer_classifies_compile_errors() {
        let analysis =
            TestFailureAnalyzer::analyze("cargo check", 101, "", "error[E0425]: cannot find value");
        assert_eq!(analysis.category, TestFailureCategory::Compile);
        assert!(analysis.summary.contains("compile check failed"));
    }

    #[test]
    fn analyzer_extracts_failed_tests() {
        let output = r#"
running 2 tests
...
"#;
        ...
        assert_eq!(analysis.failed_tests, vec!["tests::one"]);
    }

    #[test]
    fn allowed_test_command_rejects_shell_operators_and_path_escape() {
        for command in [
            "cargo test && rm -rf target",
            "cargo test ../outside",
            "pytest C:\\outside\\test.py",
            "npm run test | tee out",
        ] {
            let error = validate_test_command(command).expect_err("reject command");
            assert!(error.to_string().contains("outside the allowed"));
        }
    }

    #[test]
    fn allowed_test_command_accepts_known_test_runners() {
        for command in [
            ...
            validate_test_command(command).expect(command);
        }
    }
}
