...
pub(crate) fn render_chat_workbench_with_state(
...
pub(crate) fn chat_header_text(screen: &WorkbenchScreen, width: usize) -> String {
...
pub(crate) fn chat_text_fallback_body_lines(
    screen: &WorkbenchScreen,
    state: &WorkbenchScreenState,
    width: usize,
    height: usize,
    ...
    if state.command_palette_open && screen_modal_cell(screen).is_none() {
        ...
        lines.extend(
            command_palette_overlay_text(screen, state)
                .lines()
                .map(str::to_owned),
        );
...
pub(crate) fn chat_surface_styled_lines(
    screen: &WorkbenchScreen,
    state: &WorkbenchScreenState,
    width: usize,
    height: usize,
...
pub(crate) fn chat_surface_max_scroll(
    screen: &WorkbenchScreen,
    state: &WorkbenchScreenState,
    width: usize,
    height: usize,
...
pub(crate) fn chat_surface_body_height(
    screen: &WorkbenchScreen,
    state: &WorkbenchScreenState,
    height: usize,
) -> usize {
...
pub(crate) fn chat_surface_plain_lines(screen: &WorkbenchScreen, width: usize) -> Vec<String> {
...
pub(crate) fn chat_styled_line(line: String) -> Line<'static> {
...
pub(crate) fn chat_notice_display_lines(
    cell: &WorkbenchCell,
    progress_status: &str,
    width: usize,
) -> Option<Vec<String>> {
    if !chat_notice_visible(cell, progress_status) {
        return None;
    }
    ...
    Some(wrap(
        &format!("• {label}: {} {}", terminal_inline(title), detail),
        width,
    ))
}

pub(crate) fn chat_notice_visible(cell: &WorkbenchCell, progress_status: &str) -> bool {
    ...
    if progress_status == "failed" && matches!(kind.as_deref(), Some("error")) {
        return false;
    }
    true
}

pub(crate) fn notice_kind(cell: &WorkbenchCell) -> Option<String> {
    extract_token_after(&cell.detail, "notice_kind=")
}

pub(crate) fn notice_detail_text(cell: &WorkbenchCell) -> String {
...
pub(crate) fn notice_human_detail(cell: &WorkbenchCell) -> String {
    let detail = notice_detail_text(cell);
...
pub(crate) fn chat_conversation_lines(screen: &WorkbenchScreen, width: usize) -> Vec<String> {
    let mut lines = Vec::new();
    let conversation = screen
        .main
        .iter()
        .filter(|cell| {
            cell.title.starts_with("user turn=")
                || cell.title.starts_with("assistant turn=")
                || cell.title == "notice command input"
        })
        .collect::<Vec<_>>();
    for cell in conversation {
        if (cell.title.starts_with("user turn=") || cell.title == "notice command input")
            && !lines.is_empty()
        {
            lines.push(chat_turn_separator(width));
            lines.push(String::new());
        }
        if cell.title == "notice command input" {
            lines.extend(chat_command_transcript_lines(cell, width));
            lines.push(String::new());
            continue;
        }
        let prefix = if cell.title.starts_with("user ") {
            "›"
        } else if cell.title.starts_with("assistant ") {
            "•"
        } else {
            ""
        };
...
pub(crate) fn chat_command_transcript_lines(cell: &WorkbenchCell, width: usize) -> Vec<String> {
    let detail = notice_detail_text(cell);
    let command =
        extract_assignment_span(&detail, "command=", &[]).unwrap_or_else(|| detail.clone());
    chat_message_lines("›", &command, width)
}

pub(crate) fn chat_message_lines(prefix: &str, detail: &str, width: usize) -> Vec<String> {
...
pub(crate) fn chat_turn_separator(width: usize) -> String {
    "─".repeat(width.max(16))
}

pub(crate) fn chat_progress_line(screen: &WorkbenchScreen) -> Option<String> {
    ...
    match status.as_str() {
        "running" => Some("• Working (Esc or Ctrl+C to interrupt)".into()),
        "approval_pending" => Some("• Waiting for approval (Alt+A approve, Alt+D deny)".into()),
        "failed" => Some(format!(
            "Turn failed. {}",
            human_error_summary(
...
pub(crate) fn chat_composer_height(
    screen: &WorkbenchScreen,
    state: &WorkbenchScreenState,
...
pub(crate) fn chat_composer_text(
    screen: &WorkbenchScreen,
    state: &WorkbenchScreenState,
    width: usize,
    include_popup: bool,
...
pub(crate) fn draw_chat_composer(
    frame: &mut Frame<'_>,
    screen: &WorkbenchScreen,
    state: &WorkbenchScreenState,
    area: Rect,
...
pub(crate) fn chat_composer_prompt_line(screen: &WorkbenchScreen, width: usize) -> String {
...
pub(crate) fn chat_inline_popup_height(popup: &serde_json::Value) -> u16 {
...
pub(crate) fn chat_popup_title(popup: &serde_json::Value) -> &'static str {
    match chat_popup_kind(popup) {
...
pub(crate) fn chat_popup_kind(popup: &serde_json::Value) -> &str {
    popup
        .get("kind")
...
pub(crate) fn chat_popup_items(popup: &serde_json::Value) -> Vec<serde_json::Value> {
...
pub(crate) fn chat_popup_lines(popup: &serde_json::Value, width: usize) -> Vec<String> {
    match chat_popup_kind(popup) {
...
pub(crate) fn chat_command_completion_lines(
...
pub(crate) fn chat_command_completion_item_line(item: &serde_json::Value, width: usize) -> String {
    let marker = if item
        .get("selected")
    ...
    let action = json_string(item, "action_label", "Run command");
    let summary = json_string(item, "summary", "none");
    truncate_terminal_text(
        &format!("{marker} {command:<18} {action} - {summary}"),
        width,
    )
}

pub(crate) fn chat_footer_text(
    screen: &WorkbenchScreen,
    state: &WorkbenchScreenState,
    width: usize,
) -> String {
...
pub(crate) fn compact_path_label(path: &str, width: usize) -> String {
...
pub(crate) fn human_error_summary(detail: &str) -> String {
...
pub(crate) fn trim_blank_edges(mut lines: Vec<String>) -> Vec<String> {
...
pub(crate) fn short_status_detail(detail: &str) -> String {
    let detail = terminal_inline(detail);
...
pub(crate) fn truncate_terminal_text(input: &str, max_chars: usize) -> String {
    if max_chars == 0 {
        return String::new();
    }
